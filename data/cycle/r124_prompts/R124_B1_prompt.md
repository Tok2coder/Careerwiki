...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r124_activity/b1.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r124_activity/b1.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r124_activity/b1.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R124_B1 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 영화기술감독 | 176528414798038 | 영화기술감독 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "영화, 비디오물, 방송프로그램 제작 및 배급업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: kcc.go.kr(방통위) · kofice.or.kr(한국국제문화교류진흥원) · kba.or.kr(한국방송협회) · kpf.or.kr(한국언론진흥재단) · kbs.co.kr · mbc.co.kr · sbs.co.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 영화라인프로듀서(다른 세션)·영화배급관리자(다른 세션)·영화배급원(다른 세션)·영화복원원(다른 세션)·영화상영기획원(이 세션)·영화스토리보드작가(다른 세션)·영화연출스크립터(다른 세션)·영화예고편제작자(다른 세션)·영화자막제작원(다른 세션)·영화제 프로그래머(이 세션)·영화제작자(다른 세션)·영화큐레이터(다른 세션)·영화편집기사(다른 세션)·영화프로듀서(다른 세션)·영화필름인화원(이 세션)·영화필름현상원(다른 세션)·영화홍보마케터(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). **[산업 통계 인용 대표]** — 이 형제군에서 산업 공유 통계문장(시장규모·생산량 등) 직접 인용은 이 직업만 허용. |
| 2 | 영화상영기획원 | 1765284248645828 | 영화상영기획원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "창작 및 예술관련 서비스업". 특화 pool: mcst.go.kr(문화체육관광부) · kofic.or.kr(영화진흥위원회) · kocca.or.kr(한국콘텐츠진흥원) · arko.or.kr(한국문화예술위원회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 영화기술감독(이 세션)·영화라인프로듀서(다른 세션)·영화배급관리자(다른 세션)·영화배급원(다른 세션)·영화복원원(다른 세션)·영화스토리보드작가(다른 세션)·영화연출스크립터(다른 세션)·영화예고편제작자(다른 세션)·영화자막제작원(다른 세션)·영화제 프로그래머(이 세션)·영화제작자(다른 세션)·영화큐레이터(다른 세션)·영화편집기사(다른 세션)·영화프로듀서(다른 세션)·영화필름인화원(이 세션)·영화필름현상원(다른 세션)·영화홍보마케터(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 영화제 프로그래머 | 1765284228163261 | 영화제-프로그래머 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "창작 및 예술관련 서비스업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: mcst.go.kr(문화체육관광부) · kofic.or.kr(영화진흥위원회) · kocca.or.kr(한국콘텐츠진흥원) · arko.or.kr(한국문화예술위원회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 영화기술감독(이 세션)·영화라인프로듀서(다른 세션)·영화배급관리자(다른 세션)·영화배급원(다른 세션)·영화복원원(다른 세션)·영화상영기획원(이 세션)·영화스토리보드작가(다른 세션)·영화연출스크립터(다른 세션)·영화예고편제작자(다른 세션)·영화자막제작원(다른 세션)·영화제작자(다른 세션)·영화큐레이터(다른 세션)·영화편집기사(다른 세션)·영화프로듀서(다른 세션)·영화필름인화원(이 세션)·영화필름현상원(다른 세션)·영화홍보마케터(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 영화필름인화원 | 1765285556372633 | 영화필름인화원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "영화, 비디오물, 방송프로그램 제작 및 배급업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: kcc.go.kr(방통위) · kofice.or.kr(한국국제문화교류진흥원) · kba.or.kr(한국방송협회) · kpf.or.kr(한국언론진흥재단) · kbs.co.kr · mbc.co.kr · sbs.co.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 영화기술감독(이 세션)·영화라인프로듀서(다른 세션)·영화배급관리자(다른 세션)·영화배급원(다른 세션)·영화복원원(다른 세션)·영화상영기획원(이 세션)·영화스토리보드작가(다른 세션)·영화연출스크립터(다른 세션)·영화예고편제작자(다른 세션)·영화자막제작원(다른 세션)·영화제 프로그래머(이 세션)·영화제작자(다른 세션)·영화큐레이터(다른 세션)·영화편집기사(다른 세션)·영화프로듀서(다른 세션)·영화필름현상원(다른 세션)·영화홍보마케터(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 5 | 예비군지휘관 | 1765284343648177 | 예비군지휘관 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "외무 및 국방 행정". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 예비박피원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). **[산업 통계 인용 대표]** — 이 형제군에서 산업 공유 통계문장(시장규모·생산량 등) 직접 인용은 이 직업만 허용. |
...
```
R124_B1 결과:
영화기술감독  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
영화상영기획원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
영화제-프로그래머  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
영화필름인화원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
예비군지휘관  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
