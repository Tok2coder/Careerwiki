...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r124_activity/b5.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r124_activity/b5.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r124_activity/b5.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R124_B5 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 영화복원원 | 1765284136633189 | 영화복원원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "영화, 비디오물, 방송프로그램 제작 및 배급업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: kcc.go.kr(방통위) · kofice.or.kr(한국국제문화교류진흥원) · kba.or.kr(한국방송협회) · kpf.or.kr(한국언론진흥재단) · kbs.co.kr · mbc.co.kr · sbs.co.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 영화기술감독(다른 세션)·영화라인프로듀서(다른 세션)·영화배급관리자(다른 세션)·영화배급원(다른 세션)·영화상영기획원(다른 세션)·영화스토리보드작가(다른 세션)·영화연출스크립터(다른 세션)·영화예고편제작자(다른 세션)·영화자막제작원(이 세션)·영화제 프로그래머(다른 세션)·영화제작자(다른 세션)·영화큐레이터(다른 세션)·영화편집기사(다른 세션)·영화프로듀서(이 세션)·영화필름인화원(다른 세션)·영화필름현상원(다른 세션)·영화홍보마케터(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 2 | 영화자막제작원 | 1765284353026140 | 영화자막제작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "영화, 비디오물, 방송프로그램 제작 및 배급업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: kcc.go.kr(방통위) · kofice.or.kr(한국국제문화교류진흥원) · kba.or.kr(한국방송협회) · kpf.or.kr(한국언론진흥재단) · kbs.co.kr · mbc.co.kr · sbs.co.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 영화기술감독(다른 세션)·영화라인프로듀서(다른 세션)·영화배급관리자(다른 세션)·영화배급원(다른 세션)·영화복원원(이 세션)·영화상영기획원(다른 세션)·영화스토리보드작가(다른 세션)·영화연출스크립터(다른 세션)·영화예고편제작자(다른 세션)·영화제 프로그래머(다른 세션)·영화제작자(다른 세션)·영화큐레이터(다른 세션)·영화편집기사(다른 세션)·영화프로듀서(이 세션)·영화필름인화원(다른 세션)·영화필름현상원(다른 세션)·영화홍보마케터(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 영화프로듀서 | 1765284228496818 | 영화프로듀서 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "영화, 비디오물, 방송프로그램 제작 및 배급업". 특화 pool: kcc.go.kr(방통위) · kofice.or.kr(한국국제문화교류진흥원) · kba.or.kr(한국방송협회) · kpf.or.kr(한국언론진흥재단) · kbs.co.kr · mbc.co.kr · sbs.co.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 영화기술감독(다른 세션)·영화라인프로듀서(다른 세션)·영화배급관리자(다른 세션)·영화배급원(다른 세션)·영화복원원(이 세션)·영화상영기획원(다른 세션)·영화스토리보드작가(다른 세션)·영화연출스크립터(다른 세션)·영화예고편제작자(다른 세션)·영화자막제작원(이 세션)·영화제 프로그래머(다른 세션)·영화제작자(다른 세션)·영화큐레이터(다른 세션)·영화편집기사(다른 세션)·영화필름인화원(다른 세션)·영화필름현상원(다른 세션)·영화홍보마케터(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 예능학원강사 | 176528404792738 | 예능학원강사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 교육기관". 특화 pool: moe.go.kr(교육부) · kice.re.kr(한국교육과정평가원) · kedi.re.kr(한국교육개발원) · nile.or.kr(국가평생교육진흥원) · kfae.re.kr(한국교육학술정보원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 예능작가(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 5 | 예술모델 | 1765284153836422 | 예술모델 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "창작 및 예술관련 서비스업". 특화 pool: mcst.go.kr(문화체육관광부) · kofic.or.kr(영화진흥위원회) · kocca.or.kr(한국콘텐츠진흥원) · arko.or.kr(한국문화예술위원회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R124_B5 결과:
영화복원원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
영화자막제작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
영화프로듀서  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
예능학원강사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
예술모델  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
