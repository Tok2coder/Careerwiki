...
- **STEP 0 (작업 시작 즉시, 첫 전공 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/m0_activity/b1.json --status running`
- **STEP LAST (세션 종료 직전, 모든 전공 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/m0_activity/b1.json --status done --tool-calls <대략 tool-call 수> --detail "<완료전공수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/m0_activity/b1.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 전공 (M0_B1 — ENHANCE 모드, marker 미보유 신규, 5전공-1세션 배치)
...
```
M0_B1 결과:
컴퓨터공학과  | rev=NNNN | distinct=NN | totalE=NN | CLEAN | 마커OK
기계공학과  | rev=NNNN | distinct=NN | totalE=NN | CLEAN | 마커OK
사회복지학과  | rev=NNNN | distinct=NN | totalE=NN | CLEAN | 마커OK
유아교육학과  | rev=NNNN | distinct=NN | totalE=NN | CLEAN | 마커OK
식품영양학과  | rev=NNNN | distinct=NN | totalE=NN | CLEAN | 마커OK

MAJORS DONE: 5/5 ok   (미완 시: MAJORS: N/5 ok, 미완=<slug + 사유>)
```
