...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r88_activity/b5.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r88_activity/b5.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r88_activity/b5.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R88_B5 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 석유화학물질분석원 | 1765283810657281 | 석유화학물질분석원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "기초 화학물질 제조업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 석유화학생산관리자 | 176528428891079 | 석유화학생산관리자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "기초 화학물질 제조업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 석유화학생산설비운전원 | 176528505584356 | 석유화학생산설비운전원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "기초 화학물질 제조업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 석유화학설비관리자 | 1765283913755119 | 석유화학설비관리자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "기초 화학물질 제조업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 석유화학설비유지보수원 | 1765284677303951 | 석유화학설비유지보수원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "기초 화학물질 제조업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R88_B5 결과:
석유화학물질분석원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
석유화학생산관리자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
석유화학생산설비운전원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
석유화학설비관리자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
석유화학설비유지보수원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
