...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r90_activity/b3.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r90_activity/b3.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r90_activity/b3.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R90_B3 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 선거작업원 | 1765285434764716 | 선거작업원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "선박 및 보트 건조업". 특화 pool: kmof.go.kr(해양수산부) · kmi.re.kr(한국해양수산개발원) · ksr.or.kr(한국선급) · portmis.go.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 선견원 | 1765284987316268 | 선견원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "방적 및 가공사 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 선광관리자 | 1765283575248773 | 선광관리자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "석탄 광업 / [B061]철 광업 / [B062]비철금속 광업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 선광기술자 | 1765283960431350 | 선광기술자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "석탄 광업 / [B061]철 광업 / [B062]비철금속 광업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 선교사 | 1765284031051654 | 선교사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "기타 협회 및 단체". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. map 특화 pool 없음 — 위 산업분류 텍스트 기준으로 소관부처(.go.kr)/협회·학회(.or.kr) deep page 직접 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R90_B3 결과:
선거작업원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
선견원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
선광관리자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
선광기술자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
선교사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
