...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r131_activity/b3.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r131_activity/b3.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r131_activity/b3.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R131_B3 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 원자력연료제조원 | 1765285274834373 | 원자력연료제조원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "기초 화학물질 제조업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 원자력설비설계원(다른 세션)·원자력안전관리원(다른 세션)·원자력품질관리원(다른 세션)·원자로공학연구원(다른 세션)·원자로운전원(다른 세션)·원자로현장운전원(다른 세션)·원자재품질분석원(이 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 2 | 원자재품질분석원 | 1765284291424628 | 원자재품질분석원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 원자력설비설계원(다른 세션)·원자력안전관리원(다른 세션)·원자력연료제조원(이 세션)·원자력품질관리원(다른 세션)·원자로공학연구원(다른 세션)·원자로운전원(다른 세션)·원자로현장운전원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 원전안전분석기술연구원 | 17652838525311 | 원전안전분석기술연구원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "전기업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: kepco.co.kr(한국전력) · khnp.co.kr(한국수력원자력) · kemco.or.kr(한국에너지공단) · motie.go.kr(산업통상자원부) · energy.or.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 원전계전기술연구원(다른 세션)·원전설비연구원(다른 세션)·원전연료장전원(다른 세션)·원전토건기술연구원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 원통도금원 | 1765285202157669 | 원통도금원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 금속 가공제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 원피제육제모원 | 176528499333153 | 원피제육제모원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "가죽, 가방 및 유사제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 원피건조원(다른 세션)·원피깎기원(다른 세션)·원피분할원(다른 세션)·원피선별원(다른 세션)·원피톱밥태고조작원(다른 세션)·원피화학처리원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
...
```
R131_B3 결과:
원자력연료제조원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
원자재품질분석원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
원전안전분석기술연구원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
원통도금원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
원피제육제모원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
