...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r86_activity/b3.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r86_activity/b3.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r86_activity/b3.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R86_B3 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 상수도관망시설운영관리사 | 1765283757118598 | 상수도관망시설운영관리사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "수도업 / [F412]토목 건설업 / [M721]건축기술, 엔지니어링 및 관련 기술 서비스업". 특화 pool: molit.go.kr(국토교통부) · kict.re.kr(한국건설기술연구원) · kiaebs.or.kr(대한건축사협회) · kce.or.kr(한국건설기술인협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 상수원수질관리원 | 1765283949955694 | 상수원수질관리원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "수도업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. map 특화 pool 없음 — 위 산업분류 텍스트 기준으로 소관부처(.go.kr)/협회·학회(.or.kr) deep page 직접 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 상업교사 | 1765284035692553 | 상업교사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "중등 교육기관". 특화 pool: moe.go.kr(교육부) · kice.re.kr(한국교육과정평가원) · kedi.re.kr(한국교육개발원) · nile.or.kr(국가평생교육진흥원) · kfae.re.kr(한국교육학술정보원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 상조영업원 | 1765284495524971 | 상조영업원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "그 외 기타 개인 서비스업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 상품·공간스토리텔러 | 1765284096487998 | 상품공간스토리텔러 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "창작 및 예술관련 서비스업". 특화 pool: mcst.go.kr(문화체육관광부) · kofic.or.kr(영화진흥위원회) · kocca.or.kr(한국콘텐츠진흥원) · arko.or.kr(한국문화예술위원회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R86_B3 결과:
상수도관망시설운영관리사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
상수원수질관리원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
상업교사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
상조영업원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
상품공간스토리텔러  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
