...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r122_activity/b3.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r122_activity/b3.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r122_activity/b3.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R122_B3 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 열전기시설관리원 | 1765283850934238 | 열전기시설관리원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "증기, 냉·온수 및 공기조절 공급업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. map 특화 pool 없음 — 위 산업분류 텍스트 기준으로 소관부처(.go.kr)/협회·학회(.or.kr) deep page 직접 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 열차기관조사 | 176528541546474 | 열차기관조사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "철도 운송업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: molit.go.kr · klta.or.kr(한국물류협회) · korcham.net(대한상의) · customs.go.kr(관세청) · kita.net(무역협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 열차검수승무원(다른 세션)·열차기관사(일반)(다른 세션)·열차수송원(다른 세션)·열차승무운용원(다른 세션)·열차승무원(다른 세션)·열차승무지도원(다른 세션)·열차신호원(이 세션)·열차여객관리원(다른 세션)·열차운용원(다른 세션)·열차운전계획원(다른 세션)·열차운행계획원(다른 세션)·열차운행관리원(이 세션)·열차차량정비관리원(다른 세션)·열차홈안내원(다른 세션)·열차화물사무원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 열차신호원 | 1765285418151350 | 열차신호원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "철도 운송업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: molit.go.kr · klta.or.kr(한국물류협회) · korcham.net(대한상의) · customs.go.kr(관세청) · kita.net(무역협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 열차검수승무원(다른 세션)·열차기관사(일반)(다른 세션)·열차기관조사(이 세션)·열차수송원(다른 세션)·열차승무운용원(다른 세션)·열차승무원(다른 세션)·열차승무지도원(다른 세션)·열차여객관리원(다른 세션)·열차운용원(다른 세션)·열차운전계획원(다른 세션)·열차운행계획원(다른 세션)·열차운행관리원(이 세션)·열차차량정비관리원(다른 세션)·열차홈안내원(다른 세션)·열차화물사무원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 열차운행관리원 | 1765284316150822 | 열차운행관리원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "철도 운송업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: molit.go.kr · klta.or.kr(한국물류협회) · korcham.net(대한상의) · customs.go.kr(관세청) · kita.net(무역협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 열차검수승무원(다른 세션)·열차기관사(일반)(다른 세션)·열차기관조사(이 세션)·열차수송원(다른 세션)·열차승무운용원(다른 세션)·열차승무원(다른 세션)·열차승무지도원(다른 세션)·열차신호원(이 세션)·열차여객관리원(다른 세션)·열차운용원(다른 세션)·열차운전계획원(다른 세션)·열차운행계획원(다른 세션)·열차차량정비관리원(다른 세션)·열차홈안내원(다른 세션)·열차화물사무원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 5 | 열처리반장 | 1765285177241793 | 열처리반장 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 금속 가공제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 열처리검사원(다른 세션)·열처리원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
...
```
R122_B3 결과:
열전기시설관리원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
열차기관조사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
열차신호원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
열차운행관리원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
열처리반장  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
