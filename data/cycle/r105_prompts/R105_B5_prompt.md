...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r105_activity/b5.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r105_activity/b5.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r105_activity/b5.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R105_B5 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 시스템반도체소자공정연구원 | 1765283890329167 | 시스템반도체소자공정연구원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "반도체 제조업". 특화 pool: ksia.or.kr(한국반도체산업협회) · motie.go.kr(산업부) · keti.re.kr(한국전자기술연구원) · etri.re.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 시스템반도체소자연구원 | 17652838907095 | 시스템반도체소자연구원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "반도체 제조업". 특화 pool: ksia.or.kr(한국반도체산업협회) · motie.go.kr(산업부) · keti.re.kr(한국전자기술연구원) · etri.re.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 시스템서포트설치원 | 1765284760422168 | 시스템서포트설치원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기반조성 및 시설물 축조관련 전문공사업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 시약원 | 1765285025447753 | 시약원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "석탄 광업 / [B061]철 광업 / [B062]비철금속 광업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 시유기조작원 | 1765285226207827 | 시유기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "내화, 비내화 요업제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R105_B5 결과:
시스템반도체소자공정연구원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
시스템반도체소자연구원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
시스템서포트설치원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
시약원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
시유기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
