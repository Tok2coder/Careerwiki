...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r105_activity/b3.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r105_activity/b3.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r105_activity/b3.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R105_B3 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 시멘트품질시험원 | 1765283834836134 | 시멘트품질시험원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "시멘트, 석회, 플라스터 및 그 제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 시멘트플랜트운전원 | 1765285238458975 | 시멘트플랜트운전원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "시멘트, 석회, 플라스터 및 그 제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 시멘트플랜트현장조작원 | 1765285238923352 | 시멘트플랜트현장조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "시멘트, 석회, 플라스터 및 그 제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 시멘트휠라기조작원 | 1765285213248606 | 시멘트휠라기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "전구 및 조명장치 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 시사다큐프로그램프로듀서 | 1765284147664385 | 시사다큐프로그램프로듀서 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "영화, 비디오물, 방송프로그램 제작 및 배급업 / [J602]텔레비전 방송업". 특화 pool: kcc.go.kr(방통위) · kofice.or.kr(한국국제문화교류진흥원) · kba.or.kr(한국방송협회) · kpf.or.kr(한국언론진흥재단) · kbs.co.kr · mbc.co.kr · sbs.co.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R105_B3 결과:
시멘트품질시험원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
시멘트플랜트운전원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
시멘트플랜트현장조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
시멘트휠라기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
시사다큐프로그램프로듀서  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
