...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r132_activity/b2.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r132_activity/b2.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r132_activity/b2.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R132_B2 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 웨이퍼반사방지코팅장비조작원 | 1765285355833575 | 웨이퍼반사방지코팅장비조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "반도체 제조업". 특화 pool: ksia.or.kr(한국반도체산업협회) · motie.go.kr(산업부) · keti.re.kr(한국전자기술연구원) · etri.re.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 웨이퍼디퓨전장비조작원(다른 세션)·웨이퍼수입검사원(다른 세션)·웨이퍼전극측정원(다른 세션)·웨이퍼전극형성장비조작원(다른 세션)·웨이퍼텍스처링장비조작원(다른 세션)·웨이퍼피에스지제거장비조작원(이 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 2 | 웨이퍼피에스지제거장비조작원 | 1765285358098591 | 웨이퍼피에스지제거장비조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "반도체 제조업". 특화 pool: ksia.or.kr(한국반도체산업협회) · motie.go.kr(산업부) · keti.re.kr(한국전자기술연구원) · etri.re.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 웨이퍼디퓨전장비조작원(다른 세션)·웨이퍼반사방지코팅장비조작원(이 세션)·웨이퍼수입검사원(다른 세션)·웨이퍼전극측정원(다른 세션)·웨이퍼전극형성장비조작원(다른 세션)·웨이퍼텍스처링장비조작원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 웹소설기획자 | 1765284129477255 | 웹소설기획자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "창작 및 예술관련 서비스업". 특화 pool: mcst.go.kr(문화체육관광부) · kofic.or.kr(영화진흥위원회) · kocca.or.kr(한국콘텐츠진흥원) · arko.or.kr(한국문화예술위원회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 웹툰아트디렉터 | 1765284129990880 | 웹툰아트디렉터 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "서적, 잡지 및 기타 인쇄물 출판업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. map 특화 pool 없음 — 위 산업분류 텍스트 기준으로 소관부처(.go.kr)/협회·학회(.or.kr) deep page 직접 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 웹툰후보정작가(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). **[산업 통계 인용 대표]** — 이 형제군에서 산업 공유 통계문장(시장규모·생산량 등) 직접 인용은 이 직업만 허용. |
| 5 | 위권틀보기원 | 1765284957467673 | 위권틀보기원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "직물직조 및 직물제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 위권반장(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
...
```
R132_B2 결과:
웨이퍼반사방지코팅장비조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
웨이퍼피에스지제거장비조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
웹소설기획자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
웹툰아트디렉터  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
위권틀보기원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
