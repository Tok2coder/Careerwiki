...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r82_activity/b1.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r82_activity/b1.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r82_activity/b1.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R82_B1 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 빙과제조반장 | 176528484913988 | 빙과제조반장 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "낙농제품 및 식용빙과류 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 빙주기조작원 | 1765284849455402 | 빙주기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "낙농제품 및 식용빙과류 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 빵성형원 | 1765284568590263 | 빵성형원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 식품 제조업". 특화 pool: mfds.go.kr(식품의약품안전처) · kfia.or.kr(한국식품산업협회) · foodnara.or.kr(식품영양성분DB) · nifds.go.kr(국가식품클러스터) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 뽕밭관리원 | 1765284535302110 | 뽕밭관리원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "작물 재배업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 사료검사원 | 1765283660818688 | 사료검사원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "동물용 사료 및 조제식품 제조업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: mfds.go.kr(식품의약품안전처) · kfia.or.kr(한국식품산업협회) · foodnara.or.kr(식품영양성분DB) · nifds.go.kr(국가식품클러스터) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R82_B1 결과:
빙과제조반장  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
빙주기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
빵성형원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
뽕밭관리원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
사료검사원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
