...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r82_activity/b4.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r82_activity/b4.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r82_activity/b4.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R82_B4 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 사무용기기판매원 | 1765284511984812 | 사무용기기판매원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 상품 전문 소매업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 사무학원강사 | 1765284041182168 | 사무학원강사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 교육기관". 특화 pool: moe.go.kr(교육부) · kice.re.kr(한국교육과정평가원) · kedi.re.kr(한국교육개발원) · nile.or.kr(국가평생교육진흥원) · kfae.re.kr(한국교육학술정보원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 사물인터넷개발자 | 1765283679137976 | 사물인터넷개발자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "전기 통신업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: kepco.co.kr(한국전력) · khnp.co.kr(한국수력원자력) · kemco.or.kr(한국에너지공단) · motie.go.kr(산업통상자원부) · energy.or.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 사물인터넷사업기획자 | 1765284096247941 | 사물인터넷사업기획자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "소프트웨어 개발 및 공급업". 특화 pool: kisa.or.kr(한국인터넷진흥원) · pipc.go.kr(개인정보위) · nia.or.kr(한국지능정보사회진흥원) · spri.kr(소프트웨어정책연구소) · nipa.kr(정보통신산업진흥원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 사방작업원 | 1765284555269993 | 사방작업원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "임업". 특화 pool: rda.go.kr(농촌진흥청) · mafra.go.kr(농림축산식품부) · nifs.go.kr(국립수산과학원) · naas.go.kr(국립농업과학원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R82_B4 결과:
사무용기기판매원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
사무학원강사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
사물인터넷개발자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
사물인터넷사업기획자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
사방작업원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
