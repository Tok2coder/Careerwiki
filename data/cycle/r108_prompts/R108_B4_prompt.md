...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r108_activity/b4.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r108_activity/b4.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r108_activity/b4.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R108_B4 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 신골압출기조작원 | 1765284973335538 | 신골압출기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "신발 및 신발 부분품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신골가공원(다른 세션)·신골마무리원(다른 세션)·신골제작현장관리자(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 2 | 신문판매지국관리원 | 1765284246359574 | 신문판매지국관리원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "서적, 잡지 및 기타 인쇄물 출판업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신문제작국장(다른 세션)·신문주필(다른 세션)·신문판매국장(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 신발금형기술자 | 1765283917504602 | 신발금형기술자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "신발 및 신발 부분품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신발 디자이너(다른 세션)·신발 프로젝트 매니저(다른 세션)·신발가죽분할원(다른 세션)·신발분류원(다른 세션)·신발생산관리자(다른 세션)·신발생산관리전산시스템개발자(다른 세션)·신발성능평가사(다른 세션)·신발수선원(이 세션)·신발시험연구원(다른 세션)·신발실험실분석원(다른 세션)·신발원가전문가(다른 세션)·신발자재구매관리자(다른 세션)·신발재단사(이 세션)·신발제화원(다른 세션)·신발채산규격전문가(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 신발수선원 | 176528497408726 | 신발수선원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "신발 및 신발 부분품 제조업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신발 디자이너(다른 세션)·신발 프로젝트 매니저(다른 세션)·신발가죽분할원(다른 세션)·신발금형기술자(이 세션)·신발분류원(다른 세션)·신발생산관리자(다른 세션)·신발생산관리전산시스템개발자(다른 세션)·신발성능평가사(다른 세션)·신발시험연구원(다른 세션)·신발실험실분석원(다른 세션)·신발원가전문가(다른 세션)·신발자재구매관리자(다른 세션)·신발재단사(이 세션)·신발제화원(다른 세션)·신발채산규격전문가(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 5 | 신발재단사 | 1765284590663796 | 신발재단사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "신발 및 신발 부분품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신발 디자이너(다른 세션)·신발 프로젝트 매니저(다른 세션)·신발가죽분할원(다른 세션)·신발금형기술자(이 세션)·신발분류원(다른 세션)·신발생산관리자(다른 세션)·신발생산관리전산시스템개발자(다른 세션)·신발성능평가사(다른 세션)·신발수선원(이 세션)·신발시험연구원(다른 세션)·신발실험실분석원(다른 세션)·신발원가전문가(다른 세션)·신발자재구매관리자(다른 세션)·신발제화원(다른 세션)·신발채산규격전문가(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
...
```
R108_B4 결과:
신골압출기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
신문판매지국관리원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
신발금형기술자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
신발수선원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
신발재단사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
