...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r108_activity/b5.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r108_activity/b5.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r108_activity/b5.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R108_B5 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 신경과전문의사 | 1765283993753272 | 신경과전문의사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "병원". 특화 pool: mohw.go.kr(보건복지부) · kma.or.kr(대한의사협회) · koreanurse.or.kr(대한간호협회) · kpa.or.kr(대한약사회) · kha.or.kr(대한병원협회) · hira.or.kr(건강보험심사평가원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 신골제작현장관리자 | 1765284973614639 | 신골제작현장관리자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "신발 및 신발 부분품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신골가공원(다른 세션)·신골마무리원(다른 세션)·신골압출기조작원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 신발분류원 | 1765285594309792 | 신발분류원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "신발 및 신발 부분품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신발 디자이너(다른 세션)·신발 프로젝트 매니저(다른 세션)·신발가죽분할원(다른 세션)·신발금형기술자(다른 세션)·신발생산관리자(다른 세션)·신발생산관리전산시스템개발자(다른 세션)·신발성능평가사(다른 세션)·신발수선원(다른 세션)·신발시험연구원(이 세션)·신발실험실분석원(다른 세션)·신발원가전문가(다른 세션)·신발자재구매관리자(다른 세션)·신발재단사(다른 세션)·신발제화원(이 세션)·신발채산규격전문가(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 신발시험연구원 | 1765283988011437 | 신발시험연구원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "신발 및 신발 부분품 제조업 / [M701]자연과학 및 공학 연구개발업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신발 디자이너(다른 세션)·신발 프로젝트 매니저(다른 세션)·신발가죽분할원(다른 세션)·신발금형기술자(다른 세션)·신발분류원(이 세션)·신발생산관리자(다른 세션)·신발생산관리전산시스템개발자(다른 세션)·신발성능평가사(다른 세션)·신발수선원(다른 세션)·신발실험실분석원(다른 세션)·신발원가전문가(다른 세션)·신발자재구매관리자(다른 세션)·신발재단사(다른 세션)·신발제화원(이 세션)·신발채산규격전문가(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 5 | 신발제화원 | 1765284600124217 | 신발제화원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "신발 및 신발 부분품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 신발 디자이너(다른 세션)·신발 프로젝트 매니저(다른 세션)·신발가죽분할원(다른 세션)·신발금형기술자(다른 세션)·신발분류원(이 세션)·신발생산관리자(다른 세션)·신발생산관리전산시스템개발자(다른 세션)·신발성능평가사(다른 세션)·신발수선원(다른 세션)·신발시험연구원(이 세션)·신발실험실분석원(다른 세션)·신발원가전문가(다른 세션)·신발자재구매관리자(다른 세션)·신발재단사(다른 세션)·신발채산규격전문가(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
...
```
R108_B5 결과:
신경과전문의사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
신골제작현장관리자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
신발분류원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
신발시험연구원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
신발제화원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
