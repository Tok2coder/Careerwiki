...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r120_activity/b3.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r120_activity/b3.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r120_activity/b3.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R120_B3 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 연료전지스택컨디셔닝조작원 | 176528539617563 | 연료전지스택컨디셔닝조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "일차전지 및 축전지 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 연료전지소재부품연구원(다른 세션)·연료전지스택연구원(다른 세션)·연료전지스택품질검사원(다른 세션)·연료전지시스템연구원(다른 세션)·연료전지주변기기조립원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 2 | 연마숫돌납부착원 | 1765285239951751 | 연마숫돌납부착원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 비금속 광물제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 연마기조작원(다른 세션)·연마반장(다른 세션)·연마숫돌세공기조작원(다른 세션)·연마숫돌연마원(다른 세션)·연마숫돌품질검사원(다른 세션)·연마제배합원(다른 세션)·연마지접착제도포기조작원(이 세션)·연마포지제조원(다른 세션)·연마포후가공원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 연마지접착제도포기조작원 | 1765285240831839 | 연마지접착제도포기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 비금속 광물제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 연마기조작원(다른 세션)·연마반장(다른 세션)·연마숫돌납부착원(이 세션)·연마숫돌세공기조작원(다른 세션)·연마숫돌연마원(다른 세션)·연마숫돌품질검사원(다른 세션)·연마제배합원(다른 세션)·연마포지제조원(다른 세션)·연마포후가공원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 연사틀보기원 | 1765285061819378 | 연사틀보기원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "방적 및 가공사 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 연소기구조립원 | 1765284642023451 | 연소기구조립원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "구조용 금속제품, 탱크 및 증기발생기 제조업". 특화 pool: molit.go.kr(국토교통부) · kict.re.kr(한국건설기술연구원) · kiaebs.or.kr(대한건축사협회) · kce.or.kr(한국건설기술인협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 연소기구조립반장(다른 세션)·연소신발전연구원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
...
```
R120_B3 결과:
연료전지스택컨디셔닝조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
연마숫돌납부착원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
연마지접착제도포기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
연사틀보기원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
연소기구조립원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
