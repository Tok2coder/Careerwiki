...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r120_activity/b5.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r120_activity/b5.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r120_activity/b5.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R120_B5 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 연료전지시스템연구원 | 1765283850210355 | 연료전지시스템연구원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "일차전지 및 축전지 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 연료전지소재부품연구원(다른 세션)·연료전지스택연구원(다른 세션)·연료전지스택컨디셔닝조작원(다른 세션)·연료전지스택품질검사원(다른 세션)·연료전지주변기기조립원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 2 | 연마숫돌연마원 | 1765285240534389 | 연마숫돌연마원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 비금속 광물제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 연마기조작원(다른 세션)·연마반장(다른 세션)·연마숫돌납부착원(다른 세션)·연마숫돌세공기조작원(다른 세션)·연마숫돌품질검사원(다른 세션)·연마제배합원(다른 세션)·연마지접착제도포기조작원(다른 세션)·연마포지제조원(다른 세션)·연마포후가공원(이 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 3 | 연마포후가공원 | 1765285241642317 | 연마포후가공원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 비금속 광물제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 연마기조작원(다른 세션)·연마반장(다른 세션)·연마숫돌납부착원(다른 세션)·연마숫돌세공기조작원(다른 세션)·연마숫돌연마원(이 세션)·연마숫돌품질검사원(다른 세션)·연마제배합원(다른 세션)·연마지접착제도포기조작원(다른 세션)·연마포지제조원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 연설문작성가 | 1765284126542231 | 연설문작성가 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "창작 및 예술관련 서비스업". 특화 pool: mcst.go.kr(문화체육관광부) · kofic.or.kr(영화진흥위원회) · kocca.or.kr(한국콘텐츠진흥원) · arko.or.kr(한국문화예술위원회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 연속염색기운전원 | 1765284936164230 | 연속염색기운전원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "섬유제품 염색, 정리 및 마무리 가공업". 특화 pool: motie.go.kr · kofoti.or.kr(한국패션산업연구원) · kfda.or.kr(한국패션디자이너협회) · kdfi.or.kr(한국디자인진흥원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 연속주조보조원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). **[산업 통계 인용 대표]** — 이 형제군에서 산업 공유 통계문장(시장규모·생산량 등) 직접 인용은 이 직업만 허용. |
...
```
R120_B5 결과:
연료전지시스템연구원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
연마숫돌연마원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
연마포후가공원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
연설문작성가  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
연속염색기운전원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
