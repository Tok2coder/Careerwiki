...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r115_activity/b4.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r115_activity/b4.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r115_activity/b4.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R115_B4 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 약국처방전산입력원 | 1765284352648316 | 약국처방전산입력원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 상품 전문 소매업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 약제살포원 | 1765285617137597 | 약제살포원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "작물재배 및 축산 관련 서비스업". 특화 pool: rda.go.kr(농촌진흥청) · mafra.go.kr(농림축산식품부) · nifs.go.kr(국립수산과학원) · naas.go.kr(국립농업과학원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 약제추출기조작원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). **[산업 통계 인용 대표]** — 이 형제군에서 산업 공유 통계문장(시장규모·생산량 등) 직접 인용은 이 직업만 허용. |
| 3 | 양계장작업원 | 1765284543523780 | 양계장작업원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "축산업". 특화 pool: rda.go.kr(농촌진흥청) · mafra.go.kr(농림축산식품부) · nifs.go.kr(국립수산과학원) · naas.go.kr(국립농업과학원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 양말제조반장 | 1765284955321597 | 양말제조반장 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "의복 액세서리 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 양말검사원(다른 세션)·양말성형기조정원(다른 세션)·양말재봉사(다른 세션)·양말편직원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 5 | 양생원 | 1765285239688612 | 양생원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "시멘트, 석회, 플라스터 및 그 제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R115_B4 결과:
약국처방전산입력원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
약제살포원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
양계장작업원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
양말제조반장  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
양생원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
