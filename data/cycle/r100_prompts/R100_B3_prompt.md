...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r100_activity/b3.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r100_activity/b3.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r100_activity/b3.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R100_B3 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 수신총괄사무원 | 176528437367064 | 수신총괄사무원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "은행 및 저축기관". 특화 pool: fss.or.kr(금융감독원) · fsc.go.kr(금융위원회) · kafa.or.kr(금융투자협회) · kofia.or.kr(금융투자협회) · kfb.or.kr(은행연합회) · knia.or.kr(손해보험협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 수압대패기조작원 | 1765285486087756 | 수압대패기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "제재 및 목재 가공업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 수압시험원 | 1765284641708914 | 수압시험원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "1차 철강 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 수용가가스설비설치원 | 1765284811654221 | 수용가가스설비설치원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "연료용 가스 제조 및 배관공급업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 수용지보상원 | 1765284233932332 | 수용지보상원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "부동산 임대 및 공급업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R100_B3 결과:
수신총괄사무원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
수압대패기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
수압시험원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
수용가가스설비설치원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
수용지보상원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
