...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r103_activity/b4.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r103_activity/b4.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r103_activity/b4.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R103_B4 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 스톱모션애니메이션캐릭터제작원 | 1765284178833503 | 스톱모션애니메이션캐릭터제작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "영화, 비디오물, 방송프로그램 제작 및 배급업". 특화 pool: kcc.go.kr(방통위) · kofice.or.kr(한국국제문화교류진흥원) · kba.or.kr(한국방송협회) · kpf.or.kr(한국언론진흥재단) · kbs.co.kr · mbc.co.kr · sbs.co.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 스트링거조작원 | 176528539104152 | 스트링거조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "의료용 기기 제조업". 특화 pool: mohw.go.kr(보건복지부) · kma.or.kr(대한의사협회) · koreanurse.or.kr(대한간호협회) · kpa.or.kr(대한약사회) · kha.or.kr(대한병원협회) · hira.or.kr(건강보험심사평가원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 스틸사진사 | 1765284174054717 | 스틸사진사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "영화, 비디오물, 방송프로그램 제작 및 배급업". 특화 pool: kcc.go.kr(방통위) · kofice.or.kr(한국국제문화교류진흥원) · kba.or.kr(한국방송협회) · kpf.or.kr(한국언론진흥재단) · kbs.co.kr · mbc.co.kr · sbs.co.kr + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 스파 매니저 | 1765283592125199 | 스파-매니저 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "미용, 욕탕 및 유사 서비스업". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 스파이럴강관조관원 | 1765285172097366 | 스파이럴강관조관원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "1차 철강 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R103_B4 결과:
스톱모션애니메이션캐릭터제작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
스트링거조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
스틸사진사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
스파-매니저  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
스파이럴강관조관원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
