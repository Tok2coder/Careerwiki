...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r111_activity/b4.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r111_activity/b4.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r111_activity/b4.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R111_B4 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 아보리스트 | 1765284557617156 | 아보리스트 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "임업". 특화 pool: rda.go.kr(농촌진흥청) · mafra.go.kr(농림축산식품부) · nifs.go.kr(국립수산과학원) · naas.go.kr(국립농업과학원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 아스콘플랜트운전원 | 1765285239325233 | 아스콘플랜트운전원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "시멘트, 석회, 플라스터 및 그 제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 아스팔트루핑지제조원(다른 세션)·아스팔트믹싱플랜트조종사(다른 세션)·아스팔트살포기조종원(다른 세션)·아스팔트제조원(다른 세션)·아스팔트제조현장감독자(이 세션)·아스팔트피니셔조종사(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). **[산업 통계 인용 대표]** — 이 형제군에서 산업 공유 통계문장(시장규모·생산량 등) 직접 인용은 이 직업만 허용. |
| 3 | 아스팔트제조현장감독자 | 1765285006213357 | 아스팔트제조현장감독자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "석유 정제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 아스콘플랜트운전원(이 세션)·아스팔트루핑지제조원(다른 세션)·아스팔트믹싱플랜트조종사(다른 세션)·아스팔트살포기조종원(다른 세션)·아스팔트제조원(다른 세션)·아스팔트피니셔조종사(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 4 | 아연정액반장 | 1765285146965536 | 아연정액반장 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "1차 비철금속 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 아연말제조원(다른 세션)·아연용해반장(다른 세션)·아연용해원(다른 세션)·아연정액원(다른 세션)·아연제련원(다른 세션)·아연젤제조원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 5 | 아이스크림원료배합기조작원 | 1765284851431731 | 아이스크림원료배합기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "낙농제품 및 식용빙과류 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 아이덴티티 디자이너(다른 세션)·아이스크림냉동기조작원(다른 세션)·아이스크림성형기조작원(다른 세션)·아이스크림제조반장(다른 세션)·아이스크림제조중앙통제장치조작원(다른 세션)·아이스크림충전원(다른 세션)·아이스크림포장원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
...
```
R111_B4 결과:
아보리스트  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
아스콘플랜트운전원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
아스팔트제조현장감독자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
아연정액반장  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
아이스크림원료배합기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
