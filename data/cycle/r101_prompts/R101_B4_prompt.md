...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r101_activity/b4.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r101_activity/b4.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r101_activity/b4.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R101_B4 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 수출입화물검수운영사무원 | 1765284312395865 | 수출입화물검수운영사무원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 운송관련 서비스업". 특화 pool: molit.go.kr · klta.or.kr(한국물류협회) · korcham.net(대한상의) · customs.go.kr(관세청) · kita.net(무역협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 수출입화물검수작업사무원 | 17652843127731 | 수출입화물검수작업사무원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 운송관련 서비스업". 특화 pool: molit.go.kr · klta.or.kr(한국물류협회) · korcham.net(대한상의) · customs.go.kr(관세청) · kita.net(무역협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 수출창고관리원 | 1765284270139360 | 수출창고관리원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "보관 및 창고업". 특화 pool: molit.go.kr · klta.or.kr(한국물류협회) · korcham.net(대한상의) · customs.go.kr(관세청) · kita.net(무역협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 수치제어드릴링기조작원 | 1765285272089897 | 수치제어드릴링기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 금속 가공제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 수치제어밀링기조작원 | 1765285272442696 | 수치제어밀링기조작원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "기타 금속 가공제품 제조업". 특화 pool: motie.go.kr · koami.or.kr(한국기계산업진흥회) · kims.re.kr(한국재료연구원) · kitech.re.kr(한국생산기술연구원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R101_B4 결과:
수출입화물검수운영사무원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
수출입화물검수작업사무원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
수출창고관리원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
수치제어드릴링기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
수치제어밀링기조작원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
