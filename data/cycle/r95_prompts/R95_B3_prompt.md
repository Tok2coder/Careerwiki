...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r95_activity/b3.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r95_activity/b3.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/0 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r95_activity/b3.json --status failed --detail "<완료>/0, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R95_B3 — ENHANCE 모드, marker 미보유 신규, 0직업-1세션 배치)
...
JOBS DONE: 0/0 ok   (미완 시: JOBS: M/0 ok, 미완=<slug + 사유>)
```
