...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r74_activity/b5.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r74_activity/b5.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r74_activity/b5.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R74_B5 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 보습학원강사 | 1765284040210169 | 보습학원강사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 특화 pool: moe.go.kr(교육부) · kice.re.kr(한국교육과정평가원) · kedi.re.kr(한국교육개발원) · nile.or.kr(국가평생교육진흥원) · kfae.re.kr(한국교육학술정보원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 보안순찰원 | 1765284453438670 | 보안순찰원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 특화 pool: kisa.or.kr(한국인터넷진흥원) · pipc.go.kr(개인정보위) · nia.or.kr(한국지능정보사회진흥원) · spri.kr(소프트웨어정책연구소) · nipa.kr(정보통신산업진흥원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 보안시스템기술자 | 1765283678870544 | 보안시스템기술자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 특화 pool: kisa.or.kr(한국인터넷진흥원) · pipc.go.kr(개인정보위) · nia.or.kr(한국지능정보사회진흥원) · spri.kr(소프트웨어정책연구소) · nipa.kr(정보통신산업진흥원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 보안프로그램개발자 | 1765283728707357 | 보안프로그램개발자 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 특화 pool: kisa.or.kr(한국인터넷진흥원) · pipc.go.kr(개인정보위) · nia.or.kr(한국지능정보사회진흥원) · spri.kr(소프트웨어정책연구소) · nipa.kr(정보통신산업진흥원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 보온공 | 1765284766158685 | 보온공 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R74_B5 결과:
보습학원강사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
보안순찰원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
보안시스템기술자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
보안프로그램개발자  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
보온공  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
