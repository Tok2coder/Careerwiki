...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r116_activity/b4.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r116_activity/b4.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r116_activity/b4.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R116_B4 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 양식장작업원 | 1765284562661801 | 양식장작업원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "양식어업 및 어업관련 서비스업". 특화 pool: rda.go.kr(농촌진흥청) · mafra.go.kr(농림축산식품부) · nifs.go.kr(국립수산과학원) · naas.go.kr(국립농업과학원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 양식기술개발원(다른 세션)·양식업관리자(다른 세션)·양식장관리원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 2 | 양육비이행면접교섭지원원 | 1765284025383294 | 양육비이행면접교섭지원원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "사회보장 행정". 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 양육비이행모니터링수행원(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). **[산업 통계 인용 대표]** — 이 형제군에서 산업 공유 통계문장(시장규모·생산량 등) 직접 인용은 이 직업만 허용. |
| 3 | 양적화플래너 | 1765284271356799 | 양적화플래너 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | DB 산업분류(heroCategory): "해상 운송업". 특화 pool: molit.go.kr · klta.or.kr(한국물류협회) · korcham.net(대한상의) · customs.go.kr(관세청) · kita.net(무역협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 어묵제조원 | 1765284852893216 | 어묵제조원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "수산물 가공 및 저장 처리업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: rda.go.kr(농촌진흥청) · mafra.go.kr(농림축산식품부) · nifs.go.kr(국립수산과학원) · naas.go.kr(국립농업과학원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** 🔴 형제직업: 어묵제조반장(다른 세션). 산업 공유 통계·자격·안전(산안법 관리감독자 등) 보일러플레이트 문장을 직업명만 치환해 재사용 절대 금지(문장단위 Jaccard 검증에 걸림). 산업 공유 통계문장 직접 인용 금지 — 대표 형제가 인용하므로 이 직업은 통계 재인용 대신 **이 직업 공정·직무 맥락으로만** 서술. |
| 5 | 어염건어물제조원 | 1765284585930192 | 어염건어물제조원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 🔴 명칭 중의성 경고 — 직업명 기반 hint가 DB 산업분류와 불일치. **직업명 신호 무시**, DB 산업분류(heroCategory): "수산물 가공 및 저장 처리업". 이 산업분류 기준으로 실직무 확정 후 출처 발굴. 참고 pool: rda.go.kr(농촌진흥청) · mafra.go.kr(농림축산식품부) · nifs.go.kr(국립수산과학원) · naas.go.kr(국립농업과학원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R116_B4 결과:
양식장작업원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
양육비이행면접교섭지원원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
양적화플래너  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
어묵제조원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
어염건어물제조원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
