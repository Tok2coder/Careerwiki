...
- **STEP 0 (작업 시작 즉시, 첫 직업 처리 전):**
  `node scripts/emit-activity.cjs --file data/cycle/r73_activity/b4.json --status running`
- **STEP LAST (세션 종료 직전, 모든 직업 처리/보고 후):**
  성공: `node scripts/emit-activity.cjs --file data/cycle/r73_activity/b4.json --status done --tool-calls <대략 tool-call 수> --detail "<완료직업수>/5 done"`
  일부/실패: `node scripts/emit-activity.cjs --file data/cycle/r73_activity/b4.json --status failed --detail "<완료>/5, 미완=<slug 사유>"`

(`--tool-calls`/`--detail`는 가능하면 채우고, 모르면 생략 가능. external_id·group_key·label·model은 base 파일에 이미 박혀 있으니 건드리지 않는다.)

---

# 처리 대상 직업 (R73_B4 — ENHANCE 모드, marker 미보유 신규, 5직업-1세션 배치)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 병리학연구원 | 1765283607957429 | 병리학연구원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 2 | 병아리감별사 | 1765284549577900 | 병아리감별사 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 3 | 병원배식원 | 1765285610478678 | 병원배식원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 특화 pool: mohw.go.kr(보건복지부) · kma.or.kr(대한의사협회) · koreanurse.or.kr(대한간호협회) · kpa.or.kr(대한약사회) · kha.or.kr(대한병원협회) · hira.or.kr(건강보험심사평가원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 4 | 병원보조원 | 1765284457339836 | 병원보조원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 특화 pool: mohw.go.kr(보건복지부) · kma.or.kr(대한의사협회) · koreanurse.or.kr(대한간호협회) · kpa.or.kr(대한약사회) · kha.or.kr(대한병원협회) · hira.or.kr(건강보험심사평가원) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
| 5 | 병원보험사무원 | 1765284357442184 | 병원보험사무원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 특화 pool: fss.or.kr(금융감독원) · fsc.go.kr(금융위원회) · kafa.or.kr(금융투자협회) · kofia.or.kr(금융투자협회) · kfb.or.kr(은행연합회) · knia.or.kr(손해보험협회) + 추가 발굴. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R73_B4 결과:
병리학연구원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
병아리감별사  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
병원배식원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
병원보조원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
병원보험사무원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
