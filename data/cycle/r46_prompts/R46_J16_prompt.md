...
# 처리 대상 직업 (R46_J16 — ENHANCE 모드, marker 미보유 신규, 1직업-1세션)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 롤러성형원 | 1765285222996671 | 롤러성형원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R46_J16 결과:
롤러성형원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
