...
1. **이 세션 할당 직업 1건 외 직업 SELECT/POST/audit 절대 X.**
   ...
   - 할당 직업 1건 처리 끝나면 즉시 종료. 새 cycle / Phase 재진입으로 자동 다른 직업 X.
...
8. **처리 후 종료 형식**: `slug | rev_id | distinct_url | totalE(정확 수치) | industry_class (niche/minor/major) | audit_status | 마커OK` 1줄 + 마지막 `JOB DONE: 1/1 ok` 또는 `JOB FAIL: <사유>` 형식.
...
20. **🔴 룰 D — tool-call 경제 (세션 사망 방지)** (2026-06-11, R43 B4 사고):
    ...
    - 같은 확인(마커·audit)을 루프로 반복 호출 금지 — audit 1회 + 마커 확인 1회로 종결, FAIL 시에만 재실행.
    - **POST 성공(revisionId 반환) 후 자가검증은 audit 1회 + 마커 1회 = 단 1라운드로 끝낸다. 통과하면 즉시 보고·종료(추가 재조회·재확인 루프 금지).** R45 J12(레이저공연기사)가 POST 완료 후 검증 루프를 206 tool-call까지 돌다 session limit 사망(POST는 됐으나 자가보고 미완) — 과검증이 사망 주원인.

---

---

# 처리 대상 직업 (R46_J10 — ENHANCE 모드, marker 미보유 신규, 1직업-1세션)

| # | name | id | slug | industry_class | URL pool hint |
|---|---|---|---|---|---|
| 1 | 로터리스크린날염기운전원 | 1765284932599457 | 로터리스크린날염기운전원 | (자체 분류: niche/major — 모호 시 default major. minor 금지: 게이트 외 분류) | 산업 소관 부처(.go.kr)·직능 협회/학회(.or.kr)·대표 기업(.co.kr) deep page + KOSIS·언론 deep article 우선. root/검색 URL 금지. **niche도 distinct≥10 필수(d<10 검증 FAIL)** |
...
```
R46_J10 결과:
로터리스크린날염기운전원  | rev=NNNN | distinct=NN | totalE=NN | class | CLEAN | 마커OK
