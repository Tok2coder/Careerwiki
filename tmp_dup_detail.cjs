...
// All pairs - we need to see the actual descriptions to judge semantically
const pairs = [
...
// Get heroIntro (short summary) which is the most reliable for semantic comparison
const sql = `SELECT name, json_extract(merged_profile_json, '$.heroIntro') as intro, substr(json_extract(merged_profile_json, '$.overviewWork.main'), 1, 200) as work FROM jobs WHERE is_active = 1 AND name IN (${inClause})`;
...
// Print each pair with both intros for human semantic review
for (const [num, cat, nameA, nameB] of pairs) {
  const rA = lookup[nameA];
  const rB = lookup[nameB];
  const introA = rA ? (rA.intro || rA.work) : null;
  const introB = rB ? (rB.intro || rB.work) : null;
  console.log(`\n=== #${num} [${cat}] ===`);
  console.log(`A: "${nameA}"`);
  console.log(`   ${trunc(introA, 150) || '(없음)'}`);
  console.log(`B: "${nameB}"`);
  console.log(`   ${trunc(introB, 150) || '(없음)'}`);
}
