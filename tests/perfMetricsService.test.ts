...
import { storePerfMetrics, type PerfMetricsPayload } from '../src/services/perfMetricsService.js'
...
class MockKV {
  ...
  async put(key: string, value: string | ArrayBuffer | ReadableStream, _options?: KVNamespacePutOptions): Promise<void> {
  ...
  async list(): Promise<{ keys: { name: string; expiration?: number | null; metadata?: unknown }[]; list_complete: boolean; cursor?: string }> {
    return {
...
const createLargePayload = (): PerfMetricsPayload => {
  const baseEvents = [
    {
      type: 'detail-action',
      action: 'x'.repeat(300),
...
const run = async () => {
  ...
  const { id } = await storePerfMetrics(kv as unknown as KVNamespace, payload, { ip: '203.0.113.5' })
  ...
  try {
    await storePerfMetrics(kv as unknown as KVNamespace, { reason: 'no-metrics' }, {})
  } catch (error) {
