...
app.get('/releases', async (c) => {
  ...
  <html lang="ko">
    ...
    <body class="bg-wiki-bg text-wiki-text min-h-screen" style="background-color:#0b1220;color:#e6e8f5;">
      ...
      <main class="max-w-[1200px] mx-auto px-4 py-20 text-center space-y-5">
        ...
        <div class="flex justify-center gap-3 pt-2">
          <a href="/help" class="px-4 py-2 rounded-lg border border-wiki-border text-sm text-wiki-text hover:bg-wiki-surface">도움말로 돌아가기</a>
          <a href="/feedback" class="px-4 py-2 rounded-lg bg-gradient-to-r from-blue-500 to-indigo-500 text-white text-sm font-semibold hover:opacity-90">건의사항</a>
        </div>
      </main>
      ${renderNavScripts()}
    </body>
  ...
  return `
    ...
    <head>
        ...
        <script>
          ...
          window.__USER__ = ${user ? JSON.stringify({ id: user.id, username: user.username, role: user.role }) : 'null'};
    ...
    <body class="bg-wiki-bg text-wiki-text min-h-screen">
        ...
        <footer class="border-t border-wiki-border/40 mt-16 bg-wiki-bg/30">
            <div class="max-w-6xl mx-auto px-6 py-8">
                <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
                    ...
                    <div class="flex flex-col gap-4 md:flex-row md:items-center md:gap-8">
                        ...
                        <nav class="flex flex-wrap items-center gap-x-6 gap-y-2 text-[13px]">
                            <a href="/analyzer" class="text-wiki-muted hover:text-white transition-colors whitespace-nowrap">AI 분석</a>
                            <a href="/job" class="text-wiki-muted hover:text-white transition-colors whitespace-nowrap">직업위키</a>
                            <a href="/major" class="text-wiki-muted hover:text-white transition-colors whitespace-nowrap">전공위키</a>
                            <a href="/howto" class="text-wiki-muted hover:text-white transition-colors whitespace-nowrap">HowTo</a>
                            <a href="/help" class="text-wiki-muted hover:text-white transition-colors whitespace-nowrap">도움말</a>
                        </nav>
                    </div>
                    
                    <!-- Contact & Copyright -->
                    <div class="flex flex-col gap-2 md:flex-row md:items-center md:gap-6 text-[13px] text-wiki-muted">
                        <a href="mailto:contact@careerwiki.org" class="inline-flex items-center gap-1.5 hover:text-wiki-primary transition-colors whitespace-nowrap">
                            <i class="fas fa-envelope text-xs opacity-70"></i>
                            <span>contact@careerwiki.org</span>
                        </a>
                        <span class="opacity-60 whitespace-nowrap">© 2024 Careerwiki</span>
                    </div>
                </div>
            </div>
        </footer>
      ...
      <div class="max-w-[1400px] mx-auto md:px-6">
        ...
        <form id="job-filter-form" data-hydration-target="job" method="get" class="mb-6">
          <div class="flex flex-col sm:flex-row gap-3">
            ...
            <!-- 정렬 + 새 직업 추가 버튼 -->
            <div class="flex items-center gap-2" id="job-hydration-toolbar">
...
(function() {
  ...
  const sortLabels = { 'relevance': '기본 순', 'salary-desc': '연봉 높은 순', 'name-asc': '이름순' };
      ...
      <div class="max-w-[1400px] mx-auto md:px-6">
        ...
        <form id="major-filter-form" data-hydration-target="major" method="get" class="mb-6">
          <div class="flex flex-col sm:flex-row gap-3">
            ...
            <!-- 정렬 + 새 전공 추가 버튼 -->
            <div class="flex items-center gap-2" id="major-hydration-toolbar">
              <div class="relative" data-dropdown="major-sort">
                <button type="button" id="major-sort-trigger" class="flex items-center gap-2 pl-4 pr-3 py-3 bg-white/[0.03] border border-white/[0.06] rounded-xl text-sm text-white/70 hover:bg-white/[0.06] hover:border-white/[0.1] focus:outline-none focus:border-wiki-secondary/40 transition-all duration-200 cursor-pointer min-w-[130px]">
...
(function() {
  const currentSort = '${sort}';
  const sortLabels = { 'relevance': '기본 순', 'employment-desc': '취업률 높은 순', 'salary-desc': '월급 높은 순', 'name-asc': '이름순' };
...
app.get('/howto', async (c) => {
  ...
  // DB에서 발행된 HowTo 가져오기 (page_type = 'guide', author_id 필수)
  const dbHowtos = await c.env.DB.prepare(`
    SELECT id, slug, title, summary, meta_data, author_id, created_at, updated_at
    FROM pages 
    WHERE page_type = 'guide' AND status = 'published' AND author_id IS NOT NULL
  ...
  // 샘플 데이터와 합치기 (DB 데이터가 먼저, 샘플은 뒤에)
  const sampleSummaries = listSampleHowtoSummaries()
  const howtoSummaries = [...dbSummaries, ...sampleSummaries]
          ...
          const tags = howto.tags && howto.tags.length
            ? `<div class="flex flex-wrap gap-2">
                ${howto.tags.slice(0, 3).map((tag: string) => 
                  `<span class="px-2.5 py-1 rounded-full bg-wiki-primary/10 text-xs font-medium text-wiki-primary">${escapeHtml(tag)}</span>`
                ).join('')}
              </div>`
            : ''
          
          const thumbnail = howto.thumbnailUrl 
            ...
            : `<div class="aspect-[16/9] bg-gradient-to-br from-wiki-primary/20 to-amber-500/10 flex items-center justify-center">
            ...
            <article class="group bg-wiki-card/50 border border-wiki-border/40 rounded-2xl overflow-hidden hover:border-wiki-primary/40 hover:shadow-lg hover:shadow-wiki-primary/5 transition-all duration-300">
              <a href="/howto/${encodeURIComponent(howto.slug)}" class="block">
                ...
                <div class="p-5 space-y-3">
                  ${tags}
                  <h2 class="text-lg font-bold text-white group-hover:text-wiki-primary transition-colors line-clamp-2">
                    ${escapeHtml(howto.title)}
                  </h2>
                  <p class="text-sm text-wiki-muted line-clamp-2 leading-relaxed">${escapeHtml(howto.snippet)}</p>
                  <div class="flex items-center justify-between pt-3 border-t border-wiki-border/30">
                    <div class="flex items-center gap-2 text-xs text-wiki-muted">
                      <span class="inline-flex items-center gap-1.5">
                        <i class="fas fa-user-circle text-wiki-primary/60"></i>
                        ${escapeHtml(howto.authorName || '운영자')}
                      </span>
                      ${dateStr ? `<span class="text-wiki-border">·</span><span>${dateStr}</span>` : ''}
                    </div>
                    <span class="text-xs text-wiki-primary font-medium group-hover:translate-x-1 transition-transform inline-flex items-center gap-1">
  ...
  const emptyState = !howtoSummaries.length ? `
    <div class="col-span-full text-center py-16">
      ...
      <a href="${user ? '/howto/write' : '/login?redirect=/howto/write'}" 
         class="inline-flex items-center gap-2 px-6 py-3 bg-wiki-primary hover:bg-wiki-primary/90 text-white font-medium rounded-xl transition">
  ...
  const content = `
    <div class="max-w-6xl mx-auto px-4 py-8">
      ...
      <header class="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-10">
        <div>
          <h1 class="text-3xl md:text-4xl font-bold text-white mb-2">HowTo 가이드</h1>
          <p class="text-wiki-muted">실전 경험에서 나온 진짜 노하우를 공유합니다</p>
        </div>
        <a href="${user ? '/howto/write' : '/login?redirect=/howto/write'}" 
           class="inline-flex items-center gap-2 px-5 py-2.5 bg-wiki-primary hover:bg-wiki-primary/90 text-white font-medium rounded-xl transition shrink-0 self-start md:self-center"
    ...
    <div class="min-h-[60vh] flex items-center justify-center px-4">
      <div class="max-w-md w-full">
        <div class="bg-wiki-card/60 border border-wiki-border/40 rounded-2xl p-8 text-center backdrop-blur-sm">
          ...
          <p class="text-xs text-wiki-muted mt-6">
            로그인하면 <a href="/terms" class="text-wiki-primary hover:underline">이용약관</a> 및 
            <a href="/privacy" class="text-wiki-primary hover:underline">개인정보처리방침</a>에 동의하게 됩니다.
          </p>
        </div>
      </div>
    </div>
              ...
              <a href="/howto/${p.slug}" class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-3 mt-2 text-xs text-wiki-muted">
                  <span>
                    ...
                    ${p.created_at ? new Date(p.created_at).toLocaleDateString('ko-KR') : '날짜 없음'}
                  </span>
  ...
  // 초안 없이 빈 에디터 페이지 렌더링
    ...
    <div class="max-w-6xl mx-auto px-4 py-6">
      ...
      <header class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
          <a href="/howto" class="text-wiki-muted hover:text-white transition">
            <i class="fas fa-arrow-left"></i>
          </a>
          <div>
            <h1 class="text-xl font-bold text-white">HowTo 가이드 작성</h1>
            <p class="text-sm text-wiki-muted" id="save-status">
              <span class="text-gray-400">●</span> 새 문서
            </p>
      </div>
        </div>
        <div class="flex items-center gap-3">
          <button type="button" id="btn-save" class="px-4 py-2 border border-wiki-border/60 text-wiki-muted hover:text-white rounded-lg transition">
            <i class="fas fa-save mr-2"></i>저장
          </button>
          <button type="button" id="btn-publish" class="px-4 py-2 bg-wiki-primary hover:bg-wiki-primary/90 text-white rounded-lg transition">
            <i class="fas fa-globe mr-2"></i>발행
          </button>
        </div>
      </header>
    ...
    <div class="max-w-6xl mx-auto px-4 py-6">
      ...
      <header class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-xl font-bold text-white">${hasPublishedVersion ? 'HowTo 가이드 편집' : 'HowTo 가이드 작성'}</h1>
            <p class="text-sm text-wiki-muted" id="save-status">
        ...
        <div class="flex items-center gap-3">
          <button type="button" id="btn-save" class="px-4 py-2 bg-wiki-card/50 border border-wiki-border/40 hover:bg-wiki-card text-white rounded-lg transition">
            <i class="fas fa-save mr-2"></i>저장
          </button>
          ${isPublished ? `
            <button type="button" id="btn-update" class="px-4 py-2 bg-wiki-primary hover:bg-wiki-primary/90 text-white rounded-lg transition">
              <i class="fas fa-sync-alt mr-2"></i>업데이트
            </button>
          ` : `
            <button type="button" id="btn-publish" class="px-4 py-2 bg-wiki-primary hover:bg-wiki-primary/90 text-white rounded-lg transition">
              <i class="fas fa-paper-plane mr-2"></i>발행
            </button>
          `}
        </div>
      </header>
...
app.get('/howto/:slug', async (c) => {
  ...
  const dbResult = await c.env.DB.prepare(`
    SELECT id, slug, title, summary, content, meta_data, author_id, status, view_count, created_at, updated_at
    FROM pages 
    WHERE page_type = 'guide' AND slug = ? AND status IN ('published', 'draft_published')
    LIMIT 1
  `).bind(slug).first()
    ...
    try {
      ...
      const howtoQuery = `
        SELECT id, slug, title, 
          COALESCE(json_extract(meta_data, '$.meta.description'), '') as snippet,
          COALESCE(json_extract(meta_data, '$.meta.keywords'), '[]') as keywords_json,
          CASE 
        ...
        WHERE page_type = 'howto'
          ...
          AND (title LIKE ? OR COALESCE(json_extract(meta_data, '$.meta.description'), '') LIKE ?${tokenConditions})
    ...
    try {
      ...
      const defaultQuery = `
        SELECT id, slug, title, 
          ...
          COALESCE(json_extract(meta_data, '$.meta.keywords'), '[]') as keywords_json
        FROM pages
        WHERE page_type = 'howto' AND status = 'published'
        ORDER BY updated_at DESC
        LIMIT 3
      `
...
const renderJobCard = (entry: { profile: any; display?: any }): string => {
  ...
  const categoryName = typeof rawCategoryName === 'string' 
    ? rawCategoryName 
    : (rawCategoryName?.value || rawCategoryName?.large || rawCategoryName?.name || '')
  ...
  try {
    ...
    return c.json({
      ...
      meta: {
        ...
        viewer: viewer ? { id: viewer.id, name: viewer.name, username: viewer.username, role: viewer.role } : null,
...
app.get('/user/comments', requireAuth, async (c) => {
  ...
  const comments = await c.env.DB.prepare(`
    SELECT c.id, c.content, c.created_at, c.page_type, c.target_slug, c.target_type,
           p.title as page_title
    FROM comments c
    LEFT JOIN pages p ON c.target_slug = p.slug AND c.page_type = 'howto'
    WHERE c.author_id = ?
    ORDER BY c.created_at DESC
    LIMIT 50
  `).bind(user.id).all<{
    id: number; content: string; created_at: string; page_type: string; 
    target_slug: string; target_type: string; page_title: string | null
  }>()
  
  const commentList = comments.results || []
  ...
  const content = `
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      ${commentList.length > 0 ? `
        <div class="space-y-4">
          ${commentList.map(c => `
            <div class="p-4 bg-wiki-card/50 border border-wiki-border/40 rounded-xl">
              <div class="flex items-start justify-between gap-4 mb-2">
                <a href="/${c.page_type}/${escapeHtml(c.target_slug)}#comments" class="text-sm text-wiki-primary hover:underline">
                  ${escapeHtml(c.page_title || c.target_slug)}
                </a>
                <span class="text-xs text-wiki-muted">${new Date(c.created_at).toLocaleDateString('ko-KR')}</span>
              </div>
              <p class="text-wiki-text text-sm line-clamp-3">${escapeHtml(c.content)}</p>
            </div>
          `).join('')}
        </div>
      ` : `
        <div class="text-center py-16">
          ...
          <h3 class="text-lg font-semibold text-white mb-2">작성한 댓글이 없습니다</h3>
          <p class="text-wiki-muted">직업, 전공, HowTo 페이지에서 댓글을 작성해보세요!</p>
        </div>
      `}
    </div>
  `
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      ${bookmarkList.length > 0 ? `
        ...
        ${jobs.length > 0 ? `
          <div class="mb-8">
            <h2 class="text-lg font-semibold text-white mb-4 flex items-center gap-2">
              <i class="fas fa-briefcase text-wiki-primary"></i> 직업 (${jobs.length})
            </h2>
            <div class="grid md:grid-cols-2 gap-3">
              ${jobs.map(b => `
                <a href="/job/${escapeHtml(b.item_slug)}" class="p-3 bg-wiki-card/50 border border-wiki-border/40 rounded-lg hover:border-wiki-primary/50 transition flex items-center justify-between">
                  <span class="text-wiki-text">${escapeHtml(b.item_title)}</span>
                  <i class="fas fa-chevron-right text-wiki-muted text-xs"></i>
                </a>
              `).join('')}
            </div>
          </div>
        ...
        ${majors.length > 0 ? `
          <div class="mb-8">
            <h2 class="text-lg font-semibold text-white mb-4 flex items-center gap-2">
              <i class="fas fa-graduation-cap text-wiki-secondary"></i> 전공 (${majors.length})
            </h2>
            <div class="grid md:grid-cols-2 gap-3">
              ${majors.map(b => `
                <a href="/major/${escapeHtml(b.item_slug)}" class="p-3 bg-wiki-card/50 border border-wiki-border/40 rounded-lg hover:border-wiki-primary/50 transition flex items-center justify-between">
                  <span class="text-wiki-text">${escapeHtml(b.item_title)}</span>
                  <i class="fas fa-chevron-right text-wiki-muted text-xs"></i>
                </a>
              `).join('')}
        ...
        ${howtos.length > 0 ? `
          <div class="mb-8">
            <h2 class="text-lg font-semibold text-white mb-4 flex items-center gap-2">
              <i class="fas fa-lightbulb text-amber-400"></i> HowTo (${howtos.length})
            </h2>
            <div class="grid md:grid-cols-2 gap-3">
              ${howtos.map(b => `
                <a href="/howto/${escapeHtml(b.item_slug)}" class="p-3 bg-wiki-card/50 border border-wiki-border/40 rounded-lg hover:border-wiki-primary/50 transition flex items-center justify-between">
                  <span class="text-wiki-text">${escapeHtml(b.item_title)}</span>
                  <i class="fas fa-chevron-right text-wiki-muted text-xs"></i>
                </a>
              `).join('')}
      ...
      ` : `
        <div class="text-center py-16">
          ...
          <h3 class="text-lg font-semibold text-white mb-2">저장한 항목이 없습니다</h3>
          <p class="text-wiki-muted mb-6">직업, 전공, HowTo 페이지에서 북마크 버튼을 눌러 저장해보세요!</p>
          <div class="flex justify-center gap-4">
            <a href="/job" class="px-4 py-2 bg-wiki-card border border-wiki-border/40 text-wiki-text rounded-lg hover:border-wiki-primary/50 transition">
              직업 둘러보기
            </a>
            <a href="/major" class="px-4 py-2 bg-wiki-card border border-wiki-border/40 text-wiki-text rounded-lg hover:border-wiki-primary/50 transition">
              전공 둘러보기
            </a>
          </div>
        </div>
        ...
        <div class="glass-card p-6 rounded-xl border border-wiki-border">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <div class="px-4 py-2 bg-wiki-card border border-wiki-border rounded-lg text-wiki-text">
                ${userData.created_at ? new Date(userData.created_at).toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric' }) : '알 수 없음'}
              </div>
            </div>
...
app.post('/api/howto/publish-direct', requireAuth, async (c) => {
  try {
    ...
    const { title, summary, contentJson, contentHtml, tags, relatedJobs, relatedMajors, relatedHowtos, thumbnailUrl } = body
    ...
    const pageId = result.meta.last_row_id ? Number(result.meta.last_row_id) : null
...
app.post('/api/howto/drafts/:id/publish', requireAuth, async (c) => {
  try {
    ...
    // 슬러그 생성/확인 (중복 시 자동으로 -2, -3 추가)
    const { generateSlug } = await import('./services/slugService')
    let baseSlug = d.slug || generateSlug(d.title)
    let slug = baseSlug
    let suffix = 2
