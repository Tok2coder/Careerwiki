...
## ⚠️ 최우선 원칙 3: API 기존 데이터 중복 금지

CareerWiki의 모든 직업/전공에는 이미 커리어넷 직업백과 API, 고용24 직업정보 API, 고용24 직업사전 API에서 끌어온 데이터가 `merged_profile_json`에 저장되어 있다.

### 절대 규칙: API에 이미 있는 필드는 건드리지 않는다

**작업 전 반드시 확인:**
1. `merged_profile_json`을 DB에서 조회하여 어떤 필드에 이미 데이터가 있는지 확인
2. 이미 데이터가 있는 필드 → **건드리지 않는다** (덮어쓰기 금지)
3. 비어있는(null) 필드만 채운다

**확인 방법:**
```bash
npx wrangler d1 execute careerwiki-kr --remote --json --command "SELECT merged_profile_json FROM jobs WHERE id = '{JOB_ID}'"
```
결과에서 top-level 키들을 확인하고, 각 필드가 null인지 데이터가 있는지 판별.

**덮어쓸 수 있는 유일한 예외:**
- API 데이터의 조사년도가 **3년 이상** 오래됐고
...
직업 상세 페이지에 **해당 직업 출신 유명인**의 실제 커리어 경로를 표시하는 기능.
...
4. 출처 내용 대조 검증 (Section 4) — WebFetch로 모든 출처의 실제 내용 확인
5. ⚠️ 각주 중복 검증 (Section 4-2) — 모든 텍스트 필드에서 [N] 중복 검사
6. 편집 API 호출 (changeSummary에 변경 사유 상세 기록)
7. 커리어트리 인물 리서치 + DB 삽입 (Section 5-3) — 적합한 인물이 있는 경우만
8. 프로덕션 확인 (Section 5)
```
...
3. QA Agent → 랜덤 3-5개 프로덕션 확인 (Section 5)
4. 결과 보고
```
