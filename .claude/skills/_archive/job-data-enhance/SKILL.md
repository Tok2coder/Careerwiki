...
| **`sidebar*` `_sources` 등록 금지** ⚠️ (2026-05-06) | `sidebarCerts` / `sidebarOrgs` / `sidebarMajors` / `sidebarJobs` 어느 것도 `_sources`에 등록 금지. 본문에 `[N]` 마커가 박히지 않는 영역(자체 `{name, url}` 객체 배열로 표시)이라 `_sources` 등록 시 글로벌 idx만 부여되고 본문 sup이 안 만들어져 **orphan 발생**. 사고 사례: 게임-기획자 `_sources.sidebarCerts[0] (id=15)` orphan (사용자 발견 2026-05-06). validate `[sidebarSources]` FAIL + audit-deep `sidebarSources` FAIL로 차단됨. URL은 sidebar 항목 자체 `url` 필드에만 |
