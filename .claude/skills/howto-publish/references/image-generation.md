...
## 5-0. 직업/전공 이미지 vs HowTo 이미지 구분

> ⚠️ **직업·전공 페이지 이미지는 `promptOverride` 사용 금지**
> - `/api/image/generate`에 `promptOverride` 없이 type+slug만 전달하면 사이트의 빌트인 프롬프트(`JOB_PROMPT_TEMPLATE`)가 자동 적용됨
> - 빌트인 프롬프트는 다크 네이비 배경, 비버 캐릭터 등 사이트 컨셉을 유지함
> - 커스텀 `promptOverride`를 넣으면 사이트 통일감이 깨짐
> - **HowTo 이미지만** 아래의 스타일 풀에서 `promptOverride` 선택·사용
...
> ⚠️ **이미지는 해당 섹션 내용과 관련 있어야 함**
> - 인라인 이미지는 배치되는 섹션의 실제 주제를 시각적으로 표현해야 함
> - 제네릭 차트·인포그래픽·추상 배경을 모든 섹션에 반복 사용하지 마라
> - 예: 체력 훈련 섹션 → 운동 장면 이미지 / 자격증 섹션 → 시험·공부 이미지
