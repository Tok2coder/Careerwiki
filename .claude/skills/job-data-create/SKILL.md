...
description: 새 직업 생성 스킬. CareerWiki에 완전히 새로운 직업을 추가할 때 사용한다. "새 직업 추가", "직업 생성", "직업 만들어줘", "job create", "없는 직업 추가", "스텁 생성", "직업 추가해줘" 등이 나오면 반드시 이 스킬을 사용한다. 기존 직업의 빈 필드를 채우는 작업은 job-data-enhance 스킬 사용.
---
...
| 8 | **URL 형식** | 직업: `https://careerwiki.org/job/슬러그` (careerwiki.**kr** 아님, `/jobs` 아님) |
| 9 | **youtubeLinks title 필수** | `[{"url":"...","title":"영상 제목"}]` — title 없으면 URL이 제목으로 노출됨. oembed 응답 title 값 사용 |
| 10 | **way vs detailReady 구분** | `way` = 페이지 "되는 방법" 서술형 텍스트 / `detailReady` = 교육과정/채용/훈련 배열 구조. 혼동 금지 |
...
> ⚠️ **직업 이미지는 `promptOverride` 사용 금지** — `promptOverride` 없이 `type`+`slug`만 전달하면 사이트 빌트인 프롬프트(`JOB_PROMPT_TEMPLATE`)가 자동 적용됨. 다크 네이비 배경, 비버 캐릭터 등 사이트 컨셉 유지. 커스텀 prompt 사용 시 사이트 통일감 깨짐.
