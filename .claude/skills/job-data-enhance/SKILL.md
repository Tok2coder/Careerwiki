...
- curriculum/recruit/training은 **배열** 필드이다.
  - 해당 배열에 데이터를 **새로 추가한 경우**: 추가한 마지막 항목의 text 끝 마침표 뒤에 인라인 [N] 마커를 붙인다.
  - 기존 데이터를 그대로 유지하고 추가하지 않은 경우: 인라인 [N] 마커 불필요.
...
— curriculum/recruit/training은 배열 필드이므로 현재 formatRichText가 적용되지 않음. 단, 데이터를 **새로 추가한 경우** 마지막 항목 text 끝 마침표 뒤에 [N] 마커를 붙인다 (sources 등록은 필수).
...
- [ ] **[self-check]** 모든 텍스트 필드의 [N]이 마침표 바로 뒤인지 육안 확인 (`합니다.[1]` ✅ / `합니다[1].` ❌ / `합니다.[1] 다음 문장` ❌)
