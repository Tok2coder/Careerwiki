...
마지막 문장이 마침표(.) 또는 완성형 어미(~다/~이다/~한다)로 끝나야 함.
완성형이더라도 경어체(~습니다/~입니다/~됩니다 등)면 별도 WARN 발생 (단, 인용/원문 맥락이면 통과).
...
- **detailReady 항목을 객체로 작성** — `{text:"..."}`, `{title,text,link}` 형태 절대 금지, 반드시 plain string
