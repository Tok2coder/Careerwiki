...
| 필드 | 분량 | 주의 |
|------|------|------|
| `way` | 200-500자, `\n` 단락 | 자격요건·시험·진입경로·실무팁. **가장 중요** |
| `detailReady.curriculum` | `string[]` 5개+ | 교육과정 |
...
| `youtubeLinks` | 2~3개 | 해당 직업 관련 YouTube 영상. oembed 실존 확인 필수. 최신(1~2년 이내) 우선 |
...
3. 직업과 직접 관련된 영상(직업 소개·자격증·취업 전망·현직자 인터뷰)만 선택
4. 최신(1~2년 이내) 영상 우선; shorts 영상도 허용
5. `youtubeLinks`: `[{url, title}]` 형식으로 2~3개 편집 API에 포함 (출처 불필요)
...
**확인 항목**: way/salary/prospect/trivia 표시 확인 | 출처 클릭 가능 URL | 각주 1~N 순차 + hover 툴팁 | `**볼드 잔류 없음**` | 기존 데이터 유지
