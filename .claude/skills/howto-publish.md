...
### 주제 선정 프로세스
```
[1] 기존 발행글 목록 반드시 DB 조회로 확인 (겹침 방지 — 아래 SQL 사용!)
...
### ⚠️ 중복 방지 (필수!)
**주제 선정 전에 반드시 아래 SQL로 기존 글 목록을 확인한다.** 제목뿐 아니라 주제/키워드가 겹치는 글이 없는지 확인해야 한다.

```sql
-- 기존 발행된 모든 HowTo 글 목록 확인
...
**중복 판정 기준**:
- 같은 시험/자격증/직업을 다루는 글이 이미 존재하면 → **발행 금지**
- "공인중개사 독학 합격" ≈ "공인중개사 시험 독학 가이드" → **같은 주제 = 중복!**
- "소방 체력시험 30점 컷" ≈ "소방 체력시험 3개월 훈련법" → **같은 주제 = 중복!**
- 다른 시험이라도 시리즈물(예: 전기기사 1편/2편)은 허용하되, 핵심 내용이 80% 이상 겹치면 중복
...
H2: 대제목 (3~5개)
  ...
  checkpointBox: 핵심 규칙/주의사항 (**글 전체에서 딱 1개만 사용!**)
  conclusionBox: 섹션 요약/핵심 정리 (**글 전체에서 딱 1개만 사용!**)
  qnaBlock: 자주 묻는 질문
...
- H2 기준 5~8개 섹션, H3 포함 시 10개 이상 소제목
- QnA 최소 4개, 테이블 최소 2개

### 리서치 깊이 (필수)
- **공식 정보만으로는 부족하다**. 반드시 합격자 후기, 커뮤니티 노하우, 실전 팁을 웹 검색으로 수집
- 검색 키워드 예시: "[주제] 합격 후기", "[주제] 독학 꿀팁", "[주제] 난이도 체감", "[주제] 교재 추천"
- 수집해야 할 정보 유형:
  1. 실제 합격자/경험자가 추천하는 교재/인강 (구체적 이름)
  2. 체감 난이도와 어려운 부분 (공식 가이드에 없는 정보)
  3. 시간 배분 전략, 실수하기 쉬운 함정
  4. 비용, 준비 기간, 현실적인 활용도
  5. 유사 자격/직업과의 비교
...
```
**규칙**: `checkpointBox`와 `conclusionBox`는 **각각 글 전체에서 딱 1개씩만** 사용한다.
- checkpointBox: 글의 가장 핵심적인 규칙/주의사항 1곳에만 배치
- conclusionBox: 글 마지막 요약/정리에 1곳에만 배치
- 중간 섹션에서 주의사항을 강조하고 싶으면 **볼드 텍스트 + bulletList** 또는 **H3 + bulletList**로 대체
