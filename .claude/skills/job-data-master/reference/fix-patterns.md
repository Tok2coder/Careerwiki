...
### `[arrayItemPeriod]` (룰 X — 2026-05-09 신규, SKILL 명시)

**원칙**: `detailReady.{curriculum,recruit,training}` array 항목 끝에 마침표(`.`) X. `[N]` 출처 마커가 항목 종결자 역할.

**fix 절차**:
1. detailReady array 항목 본문 검사
2. 끝이 `.[N]` / `다.[N]` 등 마침표 + 출처 패턴이면 마침표 제거
3. 산문 영역 (way / trivia / overviewWork.main 등) 종결 마침표는 보존 — array만 적용
...
**현재 자동 검사 X** — SKILL 명시 룰만. audit 자동화는 다음 phase (validate gate 추가).

### `[sourcePositionCluster]` (룰 Y — 2026-05-09 신규, SKILL 명시)

**원칙**: detailReady array N개 항목이 1개 출처에만 통째로 의존하면서 마지막 항목에만 [N] 박는 패턴 금지. 1 항목 = 1 [N] 마커 (그 항목 fact가 그 출처 페이지에 직접 등장 시).

**fix 절차**:
1. detailReady array 항목별 본문 fact 추출
2. 각 항목 fact를 직접 cover하는 출처 매핑 검사
3. 5 항목 모두 1 출처에만 매핑 + [N]이 마지막 항목에만 박혀있으면 FAIL
4. 옵션:
   - (a) **출처 더 발굴** — 항목별 deep page WebFetch (선호)
   - (b) **4단계 fallback** — `reference/fix-patterns.md` 4단계 fallback 적용
   - (c) **항목 일반화** — 1 출처가 cover 가능한 일반 표현으로 약화 후 1:1 매핑

```js
// 잘못된 패턴 (5 항목 모두 1 출처에 의존, [1]이 마지막에만):
detailReady: { recruit: [
  "사람인 채용 공고 모니터링",
  "기업 자체 채용 페이지 확인",
  "채용 박람회 참석",
  "헤드헌터 네트워크 활용",
  "직무 적합 키워드 탐색 [1]"      // 마지막에만 [1] 박힘
]}
sources: { "detailReady.recruit": [{id:1, url:"https://saramin.co.kr/.../article-X"}] }

// 올바른 패턴 (항목별 1:1 출처 매핑):
detailReady: { recruit: [
  "사람인 채용 공고 모니터링 [1]",
  "기업 자체 채용 페이지 확인 [2]",
  "채용 박람회 참석 [3]",
...
sources: { "detailReady.recruit": [
  {id:1, url:"https://saramin.co.kr/.../article-A"},
  {id:2, url:"https://careers.samsung.com/main"},
  {id:3, url:"https://hrdkorea.or.kr/.../jobfair-2026"},
  {id:4, url:"https://wanted.co.kr/.../headhunter-network"},
  {id:5, url:"https://jobkorea.co.kr/.../keyword-trends"},
]}
```
