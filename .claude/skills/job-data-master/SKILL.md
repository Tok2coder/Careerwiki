...
2. **Self-Report 강제 enumerate**: 17필드 화이트리스트 표 사용 금지. `scripts/skill-cache/wl-kill-diag.cjs <slug>` 실행 의무.
3. **patch 계획 검증** (Phase 2 직후): 진단 결과의 모든 fail/short field가 patch 계획에 포함됐는가? skip하려면 changeSummary에 명시적 사유 필수.
...
16. **URL count 최소 ≥ 18 — distinct URL (룰 Z, 2026-05-10 신규 / 2026-05-18 강화)** — force-enhance 후 직업당 **distinct URL count** ≥ `max(18, fieldsCount × 1.5)`. **2026-05-18 강화**: (a) target 12→18 (사용자 명시) (b) count는 **distinct URL Set** — 같은 URL N번 재사용으로 ref count 부풀리기 차단. cleanup으로 broken/wiki/root sources 제거한 만큼 신규 1차 출처 발굴 의무. **2026-05-09 사고**: 103 직업 중 6 직업이 URL <10. **2026-05-18 사고**: R1 25 직업 force-enhance 후 25/25 모두 target 미달. Phase 1 ANALYZE에 sparse fieldKey 식별 의무 + Phase 2 PATCH에 모든 sparse 보강 의무 + Phase 6 VERIFY에 distinct URL count ≥ 18 검증 (미달 시 RETRY). audit-via-api `urlCountInsufficient(N<X)` WARN level (FAIL 아님 — 다음 cycle 트리거).
...
23. **WL-KILL — 화이트리스트 폐기** (2026-05-16, PR #32) — 17필드 체크리스트 방식 진단 금지. 21+ 영역 (prose body 14 + sidebar 4 + 보조 5) 자동 enumerate (`scripts/skill-cache/wl-kill-diag.cjs`) + patch 또는 skip 사유 명시 의무. PR #31 OMEGA 사후 137건 잔존 사고 (abilities/summary/duties invisible) 재발 차단.
25. **summary fieldKey sources 등록 금지 + 자격증 카탈로그 URL reject** (2026-05-22 PR #43) — `_sources["summary"]` URL 등록 절대 X (careernet canonical, 사용자 편집 불가 영역). 자격증 카탈로그 URL (`q-net.or.kr/crf005.do?id=crf00505` / `q-net.or.kr/crf005.do?id=crf00503` / `hrdkorea.or.kr/1/2/2/N` 등 — main content < 200자 + "관련부처/시행기관" 키워드) reject. **2026-05-22 사고**: 3d프린터설치정비원 `_sources.summary[1]=q-net crf005 자격증 카테고리` 사용자 spot check — 본문 마커 careernet 원본에 등장, 사이드바 출처는 보이는데 본문 매핑 부정확. 대안: 자격증 시험 일정 deep URL (`cst006.do?...artlSeq=N`) 또는 위키 deep. `reference/safety-rules.md` 18·19 룰 참조.

24. **WL-FULL-CYCLE — 풀 사이클 100% 적용 + pool-limited 절충** (2026-05-18 신규 / 2026-05-18 절충 추가) — 배치 모드 force-enhance 시 어떤 영역도 skip 금지. **minimal patch 금지** / **"시간 부담"·"별도 사이클 위탁" 핑계 금지** / **detailReady 본문 [N] 마커 갯수 감소 금지** (이미 박힌 마커 보존 — `detectArrayMarkerCountDecrease`). 강제 처리 영역: prose body 9개 (way·summary·duties·prospect·trivia·abilities·wlb·social·overviewWork) 100자+ → [N]+sources / detailReady array 3개 모든 항목 [N]+sources 1:1 / sidebar 3개 root → deep / **distinct URL count ≥ 18 (강제 목표)**.<br/>**🆕 pool-limited 절충 (2026-05-18 추가)**: 한국 1차 deep URL pool이 한정된 도메인 (의료/상담/법무/희소 직업)에서 distinct ≥ 18 발굴 불가 시 **distinct ≥ 10 허용** — 단 다음 절충 조건 **모두** 충족 시: (1) 강제 처리 영역 100% 완료 (2) wikiQuota ≤ 30% (3) detailReady 마커 갯수 보존 (4) audit Summary 0/1 FAIL (5) 본문 fact 보존 (6) Self-Report에 "distinct N (pool 한정)" + 시도한 발굴 URL list (404 등 실패 사유) 명시. **LLM 시간 부담은 pool-limited 사유 X**.<br/>**정당한 skip**: careernet canonical 보존 (researchList·certificate·pathExplore·summary·duties·overviewWork) / 정치 risk careerTree / 본문 100자 미만.<br/>**Self-Report 형식**: `patch 영역 / skip 영역 + 사유 / distinct URL count after (pool-limited=N if applicable) / 시도 URL pool 발굴 list / detailReady 마커 처음→끝 / careerTree INSERT`.<br/>**위반**: minimal patch 보고 시 사고 간주, STOP. **2026-05-18 사고**: R1 B3 minimal patch로 R1 25/25 모두 URL count 미달 + R1-redo B2 5/5 distinct 8~11 cap. `feedback_full_cycle_no_minimal_patch.md` 참조.<br/>**관련 검출**: `detectFullCycleCompliance(findings)` — Rule 17 절충 조건 모두 체크.

26. **`_sources[].text` 한글 제목 필수 — URL 자기복제 금지** (2026-05-22 신규) — patch 작성 시 모든 `_sources[fk][i].text`는 한글 제목 (또는 한글 + 영어 혼용 부제) 명시. **`text === url` 또는 `text`에 `http(s)://` 시작 패턴 절대 금지**. **2026-05-22 사고 (사용자 발견)**: R1 B1+B2 5 직업 (AI신뢰성검증전문가·ar기획자·cam-프로그래머·cctv통합관제요원·cfx아티스트) 총 109 sources 중 ~108건이 `text` 필드에 raw URL 저장 — 사이드바 렌더가 `source.text`만 사용 (`unifiedJobDetail.ts:2686 cleanText = source.text.replace(/^\[\d+\]\s*/, '')`)하므로 raw URL이 그대로 사이드바에 노출. **근본 원인**: 서버 `job-editor.ts:557 text: text || url` silent fallback — patch에서 text 누락 시 url을 text로 복사. 차단: (a) client validate에 `detectSourceTextIsUrl` 검출 (b) server-side guard — text 누락이면 reject (c) master skill self-audit text 검증 의무. **text 작성 가이드**: "기관명 — 페이지 제목" 형식 (예: `"e4ds 뉴스 — 정밀지도 시장 4,153억 전망"`, `"HYBE — A&R 기획자 채용 공고"`). 최소 5자 이상 한글 포함 필수. validate `[sourceTextIsUrl]` FAIL.
