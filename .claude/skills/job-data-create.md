...
| ★★ | `trivia` | 흥미로운 사실 1개 |
| ★★ | `detailWlb.wlb` + `detailWlb.social` | **등급 필수** — 페이지 상단 요약 카드에 크게 표시됨. 없으면 공란으로 렌더링 |
| ★★ | `detailWlb.wlbDetail` + `detailWlb.socialDetail` | 등급 하단 상세 서술. sources 필수 |
...
### 1-0. ⚠️ 고유 통계 존재 여부 사전 검증 (최우선 게이트)

**신규 직업 생성 여부를 결정하는 핵심 기준.** 이 단계에서 "생성 불필요"로 판정되면 이후 단계 전체를 스킵하고 기존 직업 보강으로 전환한다.

**검증 질문:**
> 이 직업명만의 **공식 임금 통계** 또는 **고용 전망 통계**가 별도로 존재하는가?
> (예: 커리어넷 직업사전, 고용24 재직자 조사, 워크넷 직업정보에서 이 이름으로 독립 수록)

| 판정 | 조건 | 행동 |
|------|------|------|
| ✅ 생성 진행 | 해당 직업명의 고유 통계(임금/전망)가 공식 출처에 독립 수록됨 | 아래 단계 계속 |
| ❌ 생성 불필요 | 통계가 없거나, 상위 카테고리 통계(예: "SW기술자")를 가져다 써야 하는 상황 | **기존 유사 직업의 heroTags/summary 보강으로 전환** |

**추가 차별성 검증 (통계가 있어도 필수 확인):**
> 기존 유사 직업과 임금·전망·way 데이터가 **동일한 출처**를 사용하게 되는가?
- ❌ 3개 미만 필드에서 차별화된 고유 데이터 → 신규 생성 의미 없음, 기존 직업 보강으로 전환
- ✅ 최소 3개 필드(임금, 전망, way 등)에서 기존 직업과 다른 고유 데이터 확보 가능 → 생성 진행

> **실패 사례**: "백엔드 개발자"는 채용공고 2,659건이지만, 커리어넷/고용24에서 독립 통계 없이 "SW기술자" 통계를 공유. "웹개발자" 직업의 heroTags 보강으로 대체.
...
> ⚠️ **create API slug 패턴 주의**: `POST /api/job/create`에 `name` 필드로 전달한 직업명의 **띄어쓰기가 하이픈으로 변환되어 slug가 생성**된다. 기존 직업들은 모두 붙여쓰기 slug를 사용한다 (예: `소방관`, `웹개발자`, `임상심리사`).
>
> **따라서 create API 호출 시 직업명에 띄어쓰기를 넣지 말 것.**
>
> ```
> ❌ 잘못됨: "name": "디지털 포렌식 수사관"  → slug: "디지털-포렌식-수사관"
> ✅ 올바름: "name": "디지털포렌식수사관"    → slug: "디지털포렌식수사관"
> ```
>
> 사람 이름, 브랜드명 등 명백히 붙여써야 하는 고유명사도 공백 없이 전달한다.
...
### 9-0. ⚠️ 신규 직업 필드별 sources 필수 규칙

신규 직업은 API 데이터가 없으므로 모든 서술 필드에 sources를 포함해야 한다. **sources가 없는 필드는 편집 API를 호출하면 안 된다.**

| 필드 | sources 필수 여부 | sources 키 이름 |
|------|-----------------|----------------|
| `way` | ✅ 필수 | `"way"` |
| `overviewSalary.sal` | ✅ 필수 | `"overviewSalary.sal"` |
| `overviewProspect.main` | ✅ 필수 | `"overviewProspect.main"` |
| `trivia` | ✅ 필수 | `"trivia"` |
| `detailWlb.wlbDetail` | ✅ 필수 | `"detailWlb.wlbDetail"` |
| `detailWlb.socialDetail` | ✅ 필수 | `"detailWlb.socialDetail"` |
| `detailWlb.wlb` / `detailWlb.social` (등급) | 불필요 | — |
| `heroTags`, `detailReady`, `sidebarJobs` 등 사실 나열 | 불필요 | — |
...
**필수 규칙:**
- **`job_slug`는 해당 직업에 처음 진입하는 스테이지에만 연결** — 이전 스테이지(학생, 타 직업 등)에는 반드시 null
  - ❌ 잘못: 대학 재학 스테이지(order=0)에 job_slug 설정 → 프로덕션에서 엉뚱한 스테이지가 강조됨
  - ✅ 올바름: "직업명 취업/데뷔" 스테이지(order=N)에만 job_slug 설정
- `job_slug` 존재 여부 확인 필수: `SELECT slug FROM jobs WHERE slug='{슬러그}' AND is_active=1`
