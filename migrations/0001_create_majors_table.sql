...
CREATE INDEX IF NOT EXISTS idx_majors_careernet_id ON majors(careernet_id);
CREATE INDEX IF NOT EXISTS idx_majors_goyong24_id ON majors(goyong24_id);
CREATE INDEX IF NOT EXISTS idx_majors_is_active ON majors(is_active);

-- 전공 업데이트 로그 (무엇이 언제 변경되었는지 추적)
CREATE TABLE IF NOT EXISTS major_update_logs (
...
CREATE INDEX IF NOT EXISTS idx_major_logs_major_id ON major_update_logs(major_id);
CREATE INDEX IF NOT EXISTS idx_major_logs_updated_at ON major_update_logs(updated_at);
