...
// Helper function for logo SVG (옵션 2: 깔끔한 단색 선택 - 변경 가능)
const getLogoSVG = (size: 'large' | 'small' = 'large') => {
  ...
  return `
    <svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      <text x="${parseInt(width)/2}" y="${parseInt(height)/2 + 10}" font-family="Arial, sans-serif" font-size="${fontSize}" font-weight="bold" 
...
app.get('/logo-options', (c) => {
  return c.html(serveStatic({ path: './public/logo-options.html' }))
})
