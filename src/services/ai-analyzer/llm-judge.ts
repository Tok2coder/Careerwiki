...
export const RECOMMENDATION_ENGINE_VERSION = 'v3.25.5'  // P3-6: 서사 유사도 top3 결정적 보너스 +15 (프롬프트 수단 한계 실측 후)
...
**★★ 명시적 진로 희망 감지 (기계적으로 적용하라!)**
- 심층 답변에 "~가 되고 싶다", "~로 전환하고 싶다", "~을 준비 중이다", "학원/자격증/편입을 알아보고 있다" 같은 **명시적 진로 희망**이 있으면:
  1. 그 희망과 같은 계열의 후보 직업은 **desireScore 85 이상**으로 평가하라. 버튼 토큰과 상충해도 예외 없다.
  2. 실행 준비(학원 등록, 자격 준비, 아르바이트 시작 등)까지 언급됐다면 feasibilityScore도 +10.
  3. 그 계열 후보의 rationale/likeReason에는 반드시 해당 답변 원문을 짧게 인용하라. 예: "'요리사가 되고 싶다'고 직접 말씀하셨습니다."
- 명시적 희망은 버튼 선택보다 강한 신호다. 사용자가 문장으로 직접 밝힌 방향을 버튼 통계로 덮어쓰지 마라.
  ...
  "jobs": [
    {
      ...
      "likeReason": "(📊 속성점수 필수 인용! ⚠️ 사용자 선호와 부합하는 높은 점수만 인용 — 선호와 상충하는 낮은 점수를 likeReason에 쓰지 마세요. 상충 사실은 riskReason에!) 예: '안정을 중시한다고 하셨는데, 이 직업은 안정성 90/100이고 정시퇴근 환경이라 워라밸 85/100입니다'",
  ...
  "majors": [
    {
      ...
      "likeReason": "(📊 속성점수 필수 인용! ⚠️ 사용자 선호와 부합하는 높은 점수만 인용 — 선호와 상충하는 낮은 점수를 likeReason에 쓰지 마세요. 상충 사실은 riskReason에!) 예: '창의적 활동을 좋아한다고 하셨는데, 이 전공은 창의성 88/100이고 자율성 80/100입니다'",
