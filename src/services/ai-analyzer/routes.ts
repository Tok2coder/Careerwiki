...
analyzerRoutes.get('/sample-result', async (c) => {
  const sampleResult = {
    ...
    premium_report: {
      ...
      profileInterpretation: {
        interests: [
          { label: '기술/IT', score: 95, insight: '기술 분야에 대한 관심이 매우 높습니다.', meaning: '새로운 기술 트렌드를 탐구하고, 코드를 작성하며 시스템을 구축하는 과정에서 강한 흥미를 느낍니다. 기술이 세상을 바꾸는 방식에 매료되어 있습니다.' },
          { label: '문제해결/분석', score: 90, insight: '문제 해결 과정 자체에서 동기를 얻습니다.', meaning: '복잡한 문제를 마주했을 때 도전감을 느끼며, 논리적으로 분해하고 해결책을 찾아가는 과정 자체에서 큰 만족감을 얻습니다.' },
          { label: '창작/디자인', score: 80, insight: '창의적 표현에 대한 욕구가 있습니다.', meaning: '단순히 기능만 만드는 것이 아니라, 사용자가 실제로 좋아할 만한 경험을 설계하고 싶어합니다. 기술과 창의성의 교차점에서 가치를 발견합니다.' },
        ],
        interests_summary: '기술을 통해 문제를 해결하고 창의적인 결과물을 만들어내는 것에 가장 큰 동기를 느낍니다.',
        strengths: [
          { label: '분석적 사고', score: 92, insight: '복잡한 정보를 분석하고 패턴을 찾는 데 탁월합니다.', meaning: '방대한 정보 속에서 핵심을 빠르게 파악하고, 데이터의 패턴이나 코드의 버그를 찾아내는 데 뛰어난 능력을 보입니다. 이 능력은 소프트웨어 개발과 데이터 분석 모두에서 핵심 역량이 됩니다.' },
          { label: '빠른 학습', score: 88, insight: '새로운 기술과 개념을 빠르게 습득합니다.', meaning: '처음 접하는 프로그래밍 언어나 프레임워크도 빠르게 이해하고 실전에 적용할 수 있습니다. 급변하는 IT 업계에서 이 학습 속도는 큰 경쟁력이 됩니다.' },
          { label: '끈기', score: 82, insight: '어려운 문제에 끈기 있게 도전합니다.', meaning: '해결하기 어려운 버그나 복잡한 알고리즘 문제 앞에서 쉽게 포기하지 않습니다. 이 끈기가 남들이 해결하지 못하는 문제를 풀어내는 원동력이 됩니다.' },
        ],
        strengths_summary: '분석력, 학습 속도, 끈기의 조합은 IT 분야에서 빠르게 성장할 수 있는 핵심 자산입니다.',
        values: [
          { label: '성장', score: 90, insight: '지속적인 성장과 발전을 가장 중요하게 여깁니다.', meaning: '현재에 안주하지 않고 끊임없이 새로운 것을 배우며 발전하는 것을 삶의 핵심 가치로 둡니다. 정체된 환경보다는 도전과 학습이 있는 곳에서 동기를 느낍니다.' },
          { label: '자율성', score: 85, insight: '자율적으로 일하는 환경을 선호합니다.', meaning: '업무 방법과 시간을 스스로 관리할 수 있는 환경에서 최고의 퍼포먼스를 발휘합니다. 마이크로매니징보다는 목표 제시 후 자율에 맡기는 문화가 잘 맞습니다.' },
          { label: '전문성', score: 80, insight: '전문성을 깊이 쌓아가는 것에 가치를 둡니다.', meaning: '넓고 얕은 지식보다는 하나의 분야를 깊이 파고들어 전문가가 되는 경로를 선호합니다. 이 성향은 시니어 개발자나 기술 스페셜리스트로의 성장에 유리합니다.' },
        ],
        values_summary: '성장, 자율성, 전문성을 중시하는 가치관은 스타트업이나 테크 기업의 문화와 높은 호환성을 보입니다.',
        constraints: [
          { label: '반복적 업무', score: 70, insight: '반복적인 업무에서 에너지가 소모됩니다.', meaning: '반복적이고 단조로운 업무가 계속되면 동기와 에너지가 크게 떨어집니다. 자동화하거나 개선할 수 없는 단순 반복 작업은 장기적으로 지속하기 어렵습니다.' },
        ],
        constraints_summary: '반복적인 업무를 피하고 싶은 성향은 커리어 선택 시 반드시 고려해야 할 요소입니다.',
      },
      metaCognition: {
        myArsenal: {
          strengths: [
            { trait: '분석적 사고', confidence: 0.92, evidence: '복잡한 문제를 체계적으로 분석하는 능력', meaning: '데이터와 코드 속에서 패턴을 찾고, 복잡한 시스템을 논리적으로 분해하는 능력이 뛰어납니다. 개발과 분석 모두에서 핵심 무기가 됩니다.' },
            { trait: '빠른 학습', confidence: 0.88, evidence: '새로운 기술을 빠르게 습득하는 학습력', meaning: '새로운 프레임워크나 언어를 접해도 빠르게 이해하고 실전에 적용합니다. 급변하는 IT 업계에서 가장 큰 경쟁력입니다.' },
            { trait: '끈기', confidence: 0.82, evidence: '어려운 과제에 끈기 있게 도전하는 자세', meaning: '쉽게 풀리지 않는 버그나 알고리즘 문제 앞에서도 포기하지 않고 끝까지 파고듭니다. 시니어 개발자로 성장하는 데 필수적인 자질입니다.' },
          ],
          hiddenStrengths: [
            { trait: '패턴 인식', confidence: 0.75, evidence: '데이터나 코드에서 규칙성을 빠르게 발견하는 능력이 잠재되어 있습니다.', meaning: '아직 본인이 인식하지 못할 수 있지만, 데이터나 코드에서 반복되는 패턴을 직관적으로 발견하는 능력이 있습니다. 데이터 분석이나 코드 리뷰에서 빛을 발할 잠재력입니다.' },
          ],
        },
        stressRecovery: {
          stressFactors: [
