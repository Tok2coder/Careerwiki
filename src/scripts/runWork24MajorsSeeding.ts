...
import { seedWork24Majors } from './etl/seedWork24Majors'

dotenv.config({ path: '.dev.vars' })

async function main() {
  console.log('🚀 Starting Work24 Majors Seeding Runner...')
  ...
  try {
    ...
    const result = await seedWork24Majors(db, { DB: db, GOYONG24_MAJOR_API_KEY: process.env.GOYONG24_MAJOR_API_KEY || '' }, { limit })
