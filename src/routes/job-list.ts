      ...
      <div class="max-w-[1400px] mx-auto px-2 md:px-6">
        ...
        <div class="mb-6 md:hidden relative" id="job-cat-dropdown">
          ...
          <div id="job-cat-menu" class="hidden absolute left-0 right-0 top-full mt-1 z-50 rounded-xl bg-wiki-card border border-white/[0.08] shadow-xl shadow-black/20 overflow-hidden max-h-[70vh] overflow-y-auto">
            <a href="/job" class="block px-4 py-2.5 text-base transition-colors ${!category ? 'bg-wiki-primary/15 text-wiki-secondary font-semibold' : 'text-wiki-muted hover:bg-white/[0.05] hover:text-white'}">전체</a>
            ${JOB_LARGE_CATEGORIES.map(cat => {
              const shortLabel = JOB_LARGE_SHORT_LABELS[cat]
              const isActive = category === cat
              return `<a href="/job?category=${encodeURIComponent(cat)}" class="block px-4 py-2.5 text-base transition-colors ${isActive ? 'bg-wiki-primary/15 text-wiki-secondary font-semibold' : 'text-wiki-muted hover:bg-white/[0.05] hover:text-white'}">${escapeHtml(shortLabel)}</a>`
            }).join('\n            ')}
          </div>
        </div>
        <script>document.addEventListener('click',function(e){var d=document.getElementById('job-cat-dropdown');if(d&&!d.contains(e.target)){document.getElementById('job-cat-menu').classList.add('hidden')}})</script>
        <!-- 카테고리 필터: 데스크톱 탭바 -->
