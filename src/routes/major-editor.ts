...
import { createMajor } from '../services/editService'
...
majorEditorRoutes.post('/api/major/:id/edit', requireJobMajorEdit, async (c) => {
  try {
    ...
    if (body.baseTimestamp) {
      const currentTs = majorRecord.user_last_updated_at as number | null
      if (currentTs && Number(currentTs) > Number(body.baseTimestamp)) {
        ...
        try { srvUserData = majorRecord.user_contributed_json ? JSON.parse(majorRecord.user_contributed_json as string) : {} } catch {}
        try { srvApiData = majorRecord.api_data_json ? JSON.parse(majorRecord.api_data_json as string) : {} } catch {}
        try { srvAdminData = majorRecord.admin_data_json ? JSON.parse(majorRecord.admin_data_json as string) : {} } catch {}
        const serverMerged = { ...srvApiData, ...srvUserData, ...srvAdminData }

        // fields는 아래에서 파싱하므로, body.fields에서 직접 키 추출
        const requestedFieldKeys = Object.keys(body.fields || {})
    ...
    if (body.fields && typeof body.fields === 'object') {
      ...
      const majorName = (majorRecord as any)?.name as string
      if (majorName) {
        c.executionCtx.waitUntil(
          notifyGoogleIndexing(c.env, `https://careerwiki.org/major/${encodeURIComponent(majorName)}`)
        )
      }
