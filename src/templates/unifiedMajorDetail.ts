...
export const renderUnifiedMajorDetail = ({ profile, partials, sources, existingJobSlugs, relatedMajorsByCategory, relatedHowtos, classificationData }: UnifiedMajorDetailTemplateParams): string => {
  ...
  const renderSidebarSection = (title: string, icon: string, body: string): string => {
    return `
      <section class="glass-card mobile-borderless border px-4 py-4 md:px-5 md:py-5 rounded-lg md:rounded-2xl space-y-4 bg-wiki-bg/30" data-major-sidebar-section>
