...
실제 발생한 버그와 해결 과정을 자세히 기록한 **[트러블슈팅 가이드](./TROUBLESHOOTING.md)** 를 참고하세요.

주요 문제 해결 사례:
- **API 데이터 파싱 vs Partials 반영 불일치**: 디버그 로그에는 성공하지만 실제 템플릿에 반영되지 않는 문제
- **변수 선언 누락으로 인한 데이터 머지 실패**: `normalize` 함수 내 정의되지 않은 변수 참조
- **데이터 우선순위 적용 실패**: 머지 로직은 정상이지만 중간 단계에서 데이터가 유실되는 경우
...
- ✅ 2025-11-04 **[MAJOR]** 직업 목록 시스템 완전 수정: 4가지 핵심 문제 해결 완료
  ...
  3. ✅ **한글 슬러그 URL 404 해결**: D1 name 검색 로직 추가로 `/job/가구제조-수리원` 같은 SEO 친화적 URL 정상 작동
  4. ✅ **캐시/소스 정보 숨김**: 개발자용 기술 정보를 사용자에게 노출하지 않도록 개선
  - 총 556개 직업 데이터 정상 표시, 모든 테스트 통과 확인
  - 상세 문서: `/docs/job-list-system-fix-2025-11-04.md` (문제 분석, 해결 방법, 테스트 결과 포함)
- ✅ 2025-10-27 **[CRITICAL]** 고용24 API 필드 매핑 오류 수정: API 필드명과 실제 내용이 불일치하는 문제 해결. `summary.way`가 실제로는 직업 소개 내용을, `path.technKnow`가 되는 방법 내용을 포함하고 있었음. 매핑 수정으로 올바른 내용이 올바른 필드에 표시되도록 개선. 변호사 페이지 현재 11개 필드 표시.
- ✅ 2025-10-27 직업 매트릭스 전체 필드 확장: API로 가져올 수 있는 **모든 필드 (총 26개)** 를 매트릭스에 추가하고 웹사이트 용어와 매칭되도록 상세 라벨 적용. 8개 카테고리로 구조화 (기본 정보, 분류 정보, 임금 및 전망, 역량 및 특성, 성향 및 환경, 업무활동, 교육 및 전공, 관련 정보). 새로 추가된 필드: 직업 분류 체계, 직업 만족도. 변호사 페이지 기준 12개 필드 표시 (데이터 있는 필드만 자동 표시).
