...

{
  ...
  // FAIL 케이스 1: [1] 뒤에 한글 문장이 이어짐
  const fail1 = "한국에는 임상심리사 자격 체계가 세 가지 병존한다.[1] 국가기술자격·국가전문자격·민간자격이다.";
  assert('FAIL-D1: 마지막 [1] 뒤 한글 문장 → FAIL', detectTriviaInlineFootnote(fail1),
    `결과: ${detectTriviaInlineFootnote(fail1)}`);

  // FAIL 케이스 2: [2]가 마지막이지만 뒤에 문장이 이어짐
  ...
  // PASS 케이스 1: [1]이 마지막 문장 끝에 위치
  const pass1 = "한국에는 임상심리사 자격 체계가 세 가지 병존한다. 국가기술자격·국가전문자격·민간자격이다.[1]";
  assert('PASS-D1: [1]이 마지막 문장 끝 → PASS', !detectTriviaInlineFootnote(pass1),
    `결과: ${detectTriviaInlineFootnote(pass1)}`);

  // PASS 케이스 2: [1], [2], [3] 각 문장 끝에, 마지막 [3] 뒤 텍스트 없음
  const pass2 = "첫번째 사실이다.[1] 두번째 사실이다.[2] 세번째로 이것이다.[3]";
  assert('PASS-D2: [3]이 마지막, 뒤에 텍스트 없음 → PASS', !detectTriviaInlineFootnote(pass2),
...

{
  ...
  // FAIL: trivia [1] 뒤에 문장이 이어짐
  const failD = {
    ...baseD,
    fields: {
      ...baseD.fields,
      trivia: "자격 체계가 세 가지 병존한다.[1] 이후 문장이 이어진다.",
    },
    sources: {
      ...baseD.sources,
  ...
  const rD1 = runValidate(failD);
  assertContains('FAIL-D validate: [Trivia/각주중간배치] 감지', rD1.output, '[Trivia/각주중간배치]');

  // PASS: trivia [1]이 마지막 문장 끝
  const passD = {
    ...baseD,
    fields: {
      ...baseD.fields,
      trivia: "자격 체계가 세 가지 병존한다. 이후 문장이다. 마지막 문장이다.[1]",
    },
    sources: {
      ...baseD.sources,
      trivia: [{ id: 1, text: "Q-net", url: "https://www.q-net.or.kr/" }],
  ...
  const rD2 = runValidate(passD);
  assertNotContains('PASS-D validate: [Trivia/각주중간배치] 없음', rD2.output, '[Trivia/각주중간배치]');
}
...
if (failed > 0) {
  ...
  console.log('✅ 전체 PASS — 룰 A/B/C/D 정상 작동 확인');
}
