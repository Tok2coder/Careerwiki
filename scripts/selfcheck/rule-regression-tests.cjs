...
// ─────────────────────────────────────────────────────────────────────────────
// 룰 D: detectTriviaInlineFootnote — 단위 테스트
// ─────────────────────────────────────────────────────────────────────────────
console.log('\n═══ 룰 D: detectTriviaInlineFootnote — 단위 테스트 ═══');

{
  ...
  // ── FAIL (a): 마지막 [N] 뒤에 실질 텍스트가 이어짐 ───────────────────────────
  // FAIL-D1: 단일 각주 뒤 문장 이어짐처럼 보이나, 아래는 다른 각주가 있는 케이스.
  //   "... 병존한다.[1] 국가기술자격... 민간자격이다." — 각주 1개뿐이면 위치 무관 PASS이므로
  //   2개 이상 케이스로 설계.
  const fail1 = "자격 체계가 세 가지 있다.[1] 세부 내용이 이어진다.[2] 마지막 문장이 남아있다.";
  assert('FAIL-D1: 마지막 [2] 뒤 문장 이어짐 → FAIL', !!detectTriviaInlineFootnote(fail1),
    `결과: ${detectTriviaInlineFootnote(fail1)}`);

  // FAIL-D2: [1] [2] 사이엔 문장이 있으나 [2] 뒤에 또 문장이 이어짐
  const fail2 = "첫문장.[1] 두번째 문장.[2] 세번째 문장이 이어진다.";
  assert('FAIL-D2: 마지막 [2] 뒤 문장 이어짐 → FAIL', !!detectTriviaInlineFootnote(fail2),
    `결과: ${detectTriviaInlineFootnote(fail2)}`);

  // ── FAIL (b): 각주 2개 이상이 맨 끝에 연속 몰림 ─────────────────────────────
  // FAIL-D3: 모든 문장 뒤 각주가 연속으로 붙음 → 출처 범위 불명확
  const fail3 = "문장1. 문장2. 문장3. 문장4.[1][2]";
  assert('FAIL-D3: [1][2] 맨 끝 연속 몰림 → FAIL', !!detectTriviaInlineFootnote(fail3),
    `결과: ${detectTriviaInlineFootnote(fail3)}`);

  // FAIL-D4: [1][2][3] 세 개가 맨 끝에 몰림
  const fail4 = "자격 체계가 세 가지 병존한다.[1][2][3]";
  assert('FAIL-D4: [1][2][3] 맨 끝 연속 몰림 → FAIL', !!detectTriviaInlineFootnote(fail4),
    `결과: ${detectTriviaInlineFootnote(fail4)}`);

  // ── PASS: 정상 각주 배치 ────────────────────────────────────────────────────
  // PASS-D1: [1]이 trivia의 마지막 문장 끝에 위치 (각주 1개)
  const pass1 = "자격 체계가 세 가지 병존한다. 국가기술자격·국가전문자격·민간자격이다.[1]";
  assert('PASS-D1: 각주 1개, 끝 위치 → PASS', !detectTriviaInlineFootnote(pass1),
    `결과: ${detectTriviaInlineFootnote(pass1)}`);

  // PASS-D2: 3개 각주가 각 문장 끝에 분산 배치
  const pass2 = "첫번째 사실이다.[1] 두번째 사실이다.[2] 세번째로 이것이다.[3]";
  assert('PASS-D2: [1][2][3] 분산 배치 → PASS', !detectTriviaInlineFootnote(pass2),
    `결과: ${detectTriviaInlineFootnote(pass2)}`);

  // PASS-D3: 각주 1개만 있고 뒤에 문장이 이어져도 PASS (trivia 전체를 해당 출처가 커버)
  const pass3 = "자격 체계가 세 가지 병존한다.[1] 국가기술자격·국가전문자격·민간자격이다.";
  assert('PASS-D3: 각주 1개는 위치 무관 → PASS', !detectTriviaInlineFootnote(pass3),
    `결과: ${detectTriviaInlineFootnote(pass3)}`);

  // PASS-D4: [1]은 중간에, [2]는 끝에 — [1]이 앞 문장 커버, [2]가 뒤 문장 커버
  const pass4 = "문장1. 문장2.[1] 문장3. 문장4.[2]";
  assert('PASS-D4: [1] 중간, [2] 끝 — 분산 → PASS', !detectTriviaInlineFootnote(pass4),
    `결과: ${detectTriviaInlineFootnote(pass4)}`);

  // PASS-D5: [1][2]가 중간에 묶여있지만, 뒤에 [3]이 끝에 있는 혼합 케이스 → PASS
  //   (중간 몰림은 [1]+[2]가 같은 범위 커버 의미로 허용)
  const pass5 = "문장1. 문장2.[1][2] 문장3.[3]";
  assert('PASS-D5: [1][2] 중간 묶음 + [3] 끝 → PASS', !detectTriviaInlineFootnote(pass5),
    `결과: ${detectTriviaInlineFootnote(pass5)}`);
}

// ─────────────────────────────────────────────────────────────────────────────
// 룰 D: validate-job-edit.cjs 통합 테스트
// ─────────────────────────────────────────────────────────────────────────────
console.log('\n═══ 룰 D: validate 통합 — Trivia/각주배치 ═══');

{
  const baseD = {
    fields: {
      way: "이 직업이 되려면 관련 학과를 졸업해야 한다.[1]",
      detailReady: { curriculum: ["관련 학과 졸업[1]"], recruit: ["대기업 채용[1]"], training: ["교육과정 이수[1]"] },
    },
    sources: {
      way: [{ id: 1, text: "커리어넷", url: "https://www.career.go.kr/cnet/front/base/job/jobView.do?SEQ=100" }],
      "detailReady.curriculum": [{ id: 1, text: "커리어넷", url: "https://www.career.go.kr/cnet/front/base/job/jobView.do?SEQ=100" }],
      "detailReady.recruit": [{ id: 1, text: "커리어넷", url: "https://www.career.go.kr/cnet/front/base/job/jobView.do?SEQ=100" }],
      "detailReady.training": [{ id: 1, text: "커리어넷", url: "https://www.career.go.kr/cnet/front/base/job/jobView.do?SEQ=100" }],
    },
    changeSummary: "룰 D 테스트",
  };

  // FAIL-a: trivia 마지막 [N] 뒤 문장 이어짐 (각주 2개 이상 케이스)
  const failA = {
    ...baseD,
    fields: {
      ...baseD.fields,
      trivia: "자격 체계가 세 가지 병존한다.[1] 세부 내용이 이어진다.[2] 마지막 문장이 남아있다.",
    },
    sources: {
      ...baseD.sources,
      trivia: [
        { id: 1, text: "Q-net", url: "https://www.q-net.or.kr/" },
        { id: 2, text: "한국임상심리학회", url: "https://www.kcp.or.kr/" },
      ],
    },
  };
  const rDa = runValidate(failA);
  assertContains('FAIL-Da validate: 마지막 [N] 뒤 문장 이어짐 → [Trivia/각주배치] 감지', rDa.output, '[Trivia/각주배치]');

  // FAIL-b: trivia [1][2] 맨 끝 연속 몰림
  const failB = {
    ...baseD,
    fields: {
      ...baseD.fields,
      trivia: "자격 체계가 세 가지 병존한다. 국가기술·국가전문·민간자격이다.[1][2]",
    },
    sources: {
      ...baseD.sources,
      trivia: [
  ...
  const rDb = runValidate(failB);
  assertContains('FAIL-Db validate: 각주 맨 끝 연속 몰림 → [Trivia/각주배치] 감지', rDb.output, '[Trivia/각주배치]');

  // PASS-1: trivia 각주 1개, 뒤에 문장이 있어도 PASS
  const pass1 = {
    ...baseD,
    fields: {
      ...baseD.fields,
      trivia: "자격 체계가 세 가지 병존한다.[1] 이후 문장이다. 마지막 문장이다.",
    },
    sources: {
      ...baseD.sources,
      trivia: [{ id: 1, text: "Q-net", url: "https://www.q-net.or.kr/" }],
    },
  };
  const rDp1 = runValidate(pass1);
  assertNotContains('PASS-D1 validate: 각주 1개는 위치 무관', rDp1.output, '[Trivia/각주배치]');

  // PASS-2: 각주가 분산 배치, 마지막 [N]이 끝에 위치
  const pass2 = {
    ...baseD,
    fields: {
      ...baseD.fields,
      trivia: "문장1. 문장2.[1] 문장3. 문장4.[2]",
    },
    sources: {
      ...baseD.sources,
      trivia: [
  ...
  const rDp2 = runValidate(pass2);
  assertNotContains('PASS-D2 validate: 분산 배치 + 마지막 [N] 끝 → PASS', rDp2.output, '[Trivia/각주배치]');
}
...
if (failed > 0) {
  ...
  console.log('✅ 전체 PASS — 룰 A/B/C/D 정상 작동 확인 (룰 D: 1개뿐 위치 무관 / 다개 시 마지막은 끝 + 중간 분산)');
}
