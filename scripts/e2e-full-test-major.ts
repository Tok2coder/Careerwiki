/**
 * Full E2E Test Script for AI Analyzer — Major (전공) Recommendation
 *
 * 8개 시나리오에 대해 완전한 전공 추천 파이프라인을 실행:
 * analyze → narrative-facts → round-questions×3 → round-answers×3 → recommend-major
 *
 * 실행: npx tsx scripts/e2e-full-test-major.ts
 *
 * 옵션:
 *   --scenario=major_stem_student    특정 시나리오만 실행
 *   --base-url=https://...           API 기본 URL (기본: https://careerwiki.org)
 *   --output-dir=docs/...            결과 저장 디렉토리
 */

// @ts-nocheck
import { MAJOR_TEST_SCENARIOS, type MajorTestScenario } from '../src/services/ai-analyzer/test-scenarios-major'
...
const OUTPUT_DIR = process.argv.find(a => a.startsWith('--output-dir='))?.split('=')[1] || 'docs/e2e-results/full-e2e-major'
const SPECIFIC_SCENARIO = process.argv.find(a => a.startsWith('--scenario='))?.split('=')[1]
const TIMEOUT_MS = 120_000  // 2분 (LLM 호출 포함)

// ============================================
// Hard Noise Patterns (결과 분석용)
// ============================================
const HARD_NOISE_PATTERNS_MAJOR = [
  // 전공 노이즈 패턴 — E2E 결과 분석에서만 사용 (서버 로직과 무관)
  /버섯/, /광업/, /용접/, /축산/, /임업/,
]
...
function generateSessionId(): string {
  return `e2e-major-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`
}
...
class AnswerGenerator {
  private scenario: MajorTestScenario
  private usedFallbacks: Record<string, number> = { round1: 0, round2: 0, round3: 0 }

  constructor(scenario: MajorTestScenario) {
  ...
  generateAnswer(questionText: string, roundNumber: number): {
    ...
    const defaults: Record<number, string> = {
      1: `이 분야에서 깊이 있게 공부하고 전문성을 쌓고 싶습니다.`,
      2: `지나치게 암기 위주이거나 성장 기회가 없는 학과는 피하고 싶습니다.`,
      3: `현실적인 입시 조건 내에서 최선의 선택을 하고 싶습니다.`,
    }
    return {
      answer: defaults[roundNumber] || '잘 모르겠지만 열심히 공부해보겠습니다.',
      matchType: 'default',
    }
  }
}
...
// ============================================
// Stage mapping (academicState → stage)
// ============================================
function academicStateToStage(academicState: string): string {
  switch (academicState) {
    case 'high_school_early': return 'major_high'
    case 'high_school_regular': return 'major_high'
    case 'high_school_undecided': return 'major_high'
    case 'university_freshman': return 'major_freshman'
    case 'transfer': return 'major_student'
    case 'retake': return 'major_high'
    case 'adult_learner': return 'major_graduate'
    default: return 'major_high'
  }
}
...
// Main E2E Runner
// ============================================
async function runScenario(scenario: MajorTestScenario): Promise<E2EResult> {
  ...
  console.log(`\n${'='.repeat(60)}`)
  console.log(`🎯 [전공] 시나리오: ${scenario.name} (${scenario.id})`)
  console.log(`   ${scenario.description}`)
  console.log(`   Session: ${sessionId}`)
  console.log(`   학업상태: ${scenario.academicState}`)
  ...
  const result: E2EResult = {
    scenario: {
      ...
      academicState: scenario.academicState,
    },
  ...
  try {
    const analyzePayload = {
      ...
      analysis_type: 'major',
      stage: academicStateToStage(scenario.academicState),
      universal_answers: {
  ...
  // ============================================
  // Step 6: POST /v3/recommend-major (최종 전공 추천)
  // ============================================
  console.log(`\n🏆 Step 6: /v3/recommend-major (최종 전공 추천)...`)
  const t6 = Date.now()
  try {
    ...
    const recommendResponse = await apiFetch('/v3/recommend-major', recommendPayload, 180_000)  // 3분
    timing.recommendMs = Date.now() - t6
    result.rawRecommendResponse = recommendResponse
    result.recommendation.processingTimeMs = recommendResponse.duration_ms || timing.recommendMs
    result.recommendation.uxMetrics = recommendResponse.ux_metrics || null

    // Extract top 10 — response format:
    // { recommendations: { top_majors: [...] } }
    const recsObj = recommendResponse.recommendations || {}
    const recs = Array.isArray(recsObj) ? recsObj
      : Array.isArray(recsObj.top_majors) ? recsObj.top_majors
      : []
    result.recommendation.top10 = recs.slice(0, 10).map((r: any, i: number) => ({
      ...
      fieldCategory: r.field_category || 'unknown',
      summary: r.rationale || r.summary || '',
    }))

    console.log(`   ✅ 완료 (${timing.recommendMs}ms) — ${recs.length}개 추천`)
    console.log(`\n   📊 Top 10 결과:`)
    for (const major of result.recommendation.top10) {
      console.log(`      #${major.rank} ${major.majorName} — Fit:${major.fitScore} (L:${major.likeScore} C:${major.canScore} R:-${major.riskPenalty}) [${major.fieldCategory}]`)
    }

  } catch (err: any) {
    timing.recommendMs = Date.now() - t6
    errors.push(`Step 6 /v3/recommend-major 실패: ${err.message}`)
    console.error(`   ❌ 실패: ${err.message}`)
  }
  ...
  // ============================================
  console.log(`\n${'─'.repeat(60)}`)
  console.log(`📈 [전공] 시나리오 완료: ${scenario.name}`)
...
// ============================================
function generateTraceMarkdown(result: E2EResult, scenario: MajorTestScenario): string {
  const lines: string[] = []

  lines.push(`# [전공] 시나리오: ${result.scenario.name} — 전체 트레이스`)
  lines.push(``)
  lines.push(`> 생성 시각: ${result.timestamp}`)
  lines.push(`> Session: ${result.sessionId}`)
  lines.push(`> Base URL: ${result.baseUrl}`)
  lines.push(`> 총 소요시간: ${result.timing.totalMs}ms`)
  lines.push(`> 학업상태: ${result.scenario.academicState}`)
  lines.push(``)
  ...
  lines.push(`| 학업상태 (academicState) | ${scenario.academicState} |`)
  ...
  // 4. 추천 결과
  lines.push(`## 4. 전공 추천 결과`)
  lines.push(``)

  if (result.recommendation.top10.length > 0) {
    lines.push(`| 순위 | 전공명 | Fit | Like | Can | Risk | 분야 |`)
    lines.push(`|------|--------|-----|------|-----|------|------|`)
    for (const major of result.recommendation.top10) {
      lines.push(`| ${major.rank} | ${major.majorName} | ${major.fitScore} | ${major.likeScore} | ${major.canScore} | -${major.riskPenalty} | ${major.fieldCategory} |`)
    }
    lines.push(``)

    // Score spread
    const scores = result.recommendation.top10.map(m => m.fitScore)
    ...
    // Noise check
    const noiseCount = result.recommendation.top10.filter(m =>
      HARD_NOISE_PATTERNS_MAJOR.some(p => p.test(m.majorName))
    ).length
    if (noiseCount > 0) {
      lines.push(`**⚠️ 노이즈 전공 ${noiseCount}개 발견**`)
      lines.push(``)
    }
  } else {
  ...
  lines.push(`| /v3/recommend-major | ${result.timing.recommendMs} |`)
...
function generateSummaryReport(results: E2EResult[]): string {
  ...
  lines.push(`# [전공] Full E2E 테스트 결과 요약`)
  lines.push(``)
  ...
  lines.push(`## 추천 품질 요약`)
  lines.push(``)
  lines.push(`| 시나리오 | Top1 전공 | Fit#1 | Fit#10 | Spread | 에러 | 추천시간 | 총시간 |`)
  lines.push(`|----------|----------|-------|--------|--------|------|---------|--------|`)
  ...
  for (const r of results) {
    ...
    lines.push(`| ${r.scenario.name} | ${top1?.majorName || 'N/A'} | ${top1?.fitScore || 0} | ${top10?.fitScore || 0} | ${spread} | ${r.errors.length} | ${r.timing.recommendMs}ms | ${r.timing.totalMs}ms |`)
  }
  lines.push(``)
...
// ============================================
async function main() {
  console.log(`\n🚀 CareerWiki AI Analyzer — [전공] Full E2E Test`)
  console.log(`   Base URL: ${BASE_URL}`)
  console.log(`   Output: ${OUTPUT_DIR}`)
  console.log(`   시나리오: ${SPECIFIC_SCENARIO || `전체 ${MAJOR_TEST_SCENARIOS.length}개`}`)
  console.log(``)

  // Filter scenarios
  const scenarios = SPECIFIC_SCENARIO
    ? MAJOR_TEST_SCENARIOS.filter(s => s.id === SPECIFIC_SCENARIO)
    : MAJOR_TEST_SCENARIOS

  if (scenarios.length === 0) {
    console.error(`❌ 시나리오를 찾을 수 없습니다: ${SPECIFIC_SCENARIO}`)
    console.error(`   사용 가능: ${MAJOR_TEST_SCENARIOS.map(s => s.id).join(', ')}`)
    process.exit(1)
  }
  ...
  for (const scenario of scenarios) {
    try {
      ...
      const tracePath = path.join(OUTPUT_DIR, '..', '..', 'ai-quality', 'traces', `${scenario.id}-major-trace.md`)
      ...
      results.push({
        scenario: { id: scenario.id, name: scenario.name, description: scenario.description, careerState: scenario.careerState, academicState: scenario.academicState },
        timestamp: new Date().toISOString(),
        baseUrl: BASE_URL,
        sessionId: 'FAILED',
        requestId: null,
  ...
  // Final summary
  console.log(`\n${'═'.repeat(60)}`)
  console.log(`✅ [전공] Full E2E 테스트 완료`)
