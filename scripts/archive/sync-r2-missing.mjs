/**
 * miniflare-R2BucketObject에서 누락된 이미지를 Production R2로 동기화
 */
...
async function main() {
  // miniflare-R2BucketObject 폴더에서 이미지 추출
  const r2DbDir = '.wrangler/state/v3/r2/miniflare-R2BucketObject';
  const files = fs.readdirSync(r2DbDir);
  const sqliteFile = files.find(f => f.endsWith('.sqlite') && !f.includes('-shm') && !f.includes('-wal'));
  
  if (!sqliteFile) {
    console.error('❌ SQLite 파일을 찾을 수 없습니다.');
    process.exit(1);
  }
  
  const dbPath = path.join(r2DbDir, sqliteFile);
  console.log(`📂 R2 DB: ${dbPath}`);
  ...
  const objects = db.prepare(`
    ...
    FROM _mf_objects
    WHERE key LIKE 'howto/%' OR key LIKE 'profile/%'
  `).all().map(obj => {
  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    try {
      ...
      if (successCount % 10 === 0) {
        console.log(`[${i + 1}/${objects.length}] ✅ ${successCount} 성공, ${errorCount} 실패, ${skipCount} 스킵`);
      }
    } catch (error) {
      errorCount++;
      if (errorCount <= 5) {
        console.log(`❌ ${obj.key} 실패: ${error.message}`);
      }
    }
