...
// 인기 cohort 우선순위
const COHORT_PRIORITY = ['의료/보건', 'IT/SW', '디자인/예술', '금융', '학자/연구'];
...
if (args.cohort) {
  ...
  // 우선순위 cohort만 필터링 + cohort 우선순위 정렬
...
const STANDARD_RULES = `### 표준 룰 (필수 준수 — 모든 5 직업 공통)
...
9. **careerTree 정치인 제외**: 역대 대통령 절대, 다른 정치인은 직업 적합성 검토 후
10. **편집 API**: \`POST https://careerwiki.org/api/job/{id}/edit\` + 헤더 \`X-Admin-Secret: careerwiki-admin-2026\` (fields+sources 동시)
...
console.log(`=== 다음 ${PICK}개 enhance 후보 (skip=${SKIP}, ${args['cohort-priority'] ? 'cohort-priority' : (args.cohort ? `cohort=${args.cohort}` : 'default')}) ===\n`);
console.log(`5 세션 × 5 직업 분할 / 권장 stagger: ${STAGGER}초\n`);

console.log('## 직업별 권고 표 (전체 25개)\n');
console.log('| # | slug | cat | sal | missing | mode | cohort |');
console.log('|---|------|-----|-----|---------|------|--------|');
...
sessions.forEach((sess, idx) => {
  ...
  console.log('```');
  console.log(`/job-data-enhance 다음 ${sess.items.length}개 직업 enhance 처리 (cohort: ${sess.cohort}).`);
  console.log('');
  sess.items.forEach((item) => {
    const url = `https://careerwiki.org/job/${item.slug}`;
    const mode = item._src === 'B_heavy' ? '풀 사이클 (Phase 0~5)' : `--field-only=${item.field_only_csv}`;
    ...
    if (item.missing_fields.includes('careerTree')) {
      console.log(`- careerTree 인물: ${careerTreeHint(item.slug, item.cohort)}`);
    }
...
console.log(`- 세션당 진행 시간 추산: A(--field-only) 1-2분, B_light 3-5분, B_heavy 5-10분`);
console.log('');
