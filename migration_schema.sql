...
CREATE TABLE anonymous_comment_counters (
  page_id INTEGER NOT NULL PRIMARY KEY,
  next_number INTEGER DEFAULT 1,
  FOREIGN KEY (page_id) REFERENCES pages(id) ON DELETE CASCADE
);
CREATE TABLE anonymous_daily_limits (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  page_id INTEGER NOT NULL,
...
CREATE TABLE anonymous_edit_limits (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE job_match_overrides (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE TABLE howto_related_jobs (
  howto_id INTEGER NOT NULL,
  job_id TEXT NOT NULL,            
  display_order INTEGER DEFAULT 0,
...
CREATE TABLE howto_related_majors (
  howto_id INTEGER NOT NULL,
  major_id TEXT NOT NULL,          
  display_order INTEGER DEFAULT 0,
...
CREATE TABLE howto_related_howtos (
...
CREATE TABLE draft_related_jobs (
  draft_id INTEGER NOT NULL,
  job_id TEXT NOT NULL,
  display_order INTEGER DEFAULT 0,
...
CREATE TABLE draft_related_majors (
  draft_id INTEGER NOT NULL,
  major_id TEXT NOT NULL,
  display_order INTEGER DEFAULT 0,
...
CREATE TABLE draft_related_howtos (
  draft_id INTEGER NOT NULL,
  howto_id INTEGER NOT NULL,
  display_order INTEGER DEFAULT 0,
...
CREATE TABLE user_attributions (user_id INTEGER PRIMARY KEY, self_channel TEXT NOT NULL, self_channel_other TEXT, interest_state TEXT, career_state TEXT, utm_source TEXT, utm_medium TEXT, utm_campaign TEXT, utm_content TEXT, utm_term TEXT, referrer TEXT, first_touch_at INTEGER NOT NULL, FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE TABLE consents (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER NOT NULL, type TEXT NOT NULL CHECK(type IN ('terms','privacy')), version TEXT NOT NULL, consented_at INTEGER NOT NULL, ip TEXT, ua TEXT, FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE);
CREATE TABLE profanity_rules (id INTEGER PRIMARY KEY AUTOINCREMENT, pattern TEXT NOT NULL UNIQUE, replacement TEXT DEFAULT '**', is_active INTEGER DEFAULT 1, created_at INTEGER DEFAULT (strftime('%s','now')));
CREATE TABLE jobs (
    id TEXT PRIMARY KEY,
    careernet_id TEXT,
    goyong24_id TEXT,
...
CREATE TABLE confirmed_constraints (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  user_id TEXT,
  session_id TEXT NOT NULL,
...
CREATE TABLE raw_events_backup(
  id INT,
  user_id TEXT,
  session_id TEXT,
  event_type TEXT,
...
CREATE INDEX idx_serp_logs_page_action ON serp_interaction_logs(page_type, action);
CREATE INDEX idx_serp_logs_recorded_at ON serp_interaction_logs(recorded_at);
CREATE INDEX idx_serp_summary_date ON serp_interaction_daily_summary(log_date);
...
CREATE INDEX idx_ip_blocks_status ON ip_blocks(status);
...
CREATE INDEX idx_comments_anonymous_number ON comments(page_id, anonymous_number);
CREATE INDEX idx_comments_edited ON comments(is_edited, edited_at);
CREATE INDEX idx_anonymous_daily_limits_lookup ON anonymous_daily_limits(page_id, ip_hash, date);
CREATE INDEX idx_anonymous_limits ON anonymous_edit_limits(entity_type, entity_id, ip_hash, edit_date);
CREATE INDEX idx_revisions_entity ON page_revisions(entity_type, entity_id);
CREATE INDEX idx_revisions_current ON page_revisions(entity_type, entity_id, is_current);
CREATE INDEX idx_revisions_number ON page_revisions(entity_type, entity_id, revision_number);
CREATE INDEX idx_revisions_editor ON page_revisions(editor_id, editor_type);
CREATE INDEX idx_revisions_created ON page_revisions(created_at DESC);
CREATE INDEX idx_job_sources_job_id ON job_sources(job_id);
CREATE INDEX idx_job_sources_system_key ON job_sources(source_system, source_key);
CREATE INDEX idx_job_sources_codes ON job_sources(std_job_cd, keco_cd, ksoc_cd);
CREATE INDEX idx_job_sources_type ON job_sources(source_type);
CREATE INDEX idx_major_sources_major_id ON major_sources(major_id);
CREATE INDEX idx_major_sources_system_key ON major_sources(source_system, source_key);
CREATE INDEX idx_major_sources_codes ON major_sources(major_seq, department_cd);
CREATE INDEX idx_major_sources_type ON major_sources(source_type);
CREATE INDEX idx_major_sources_track ON major_sources(track);
CREATE INDEX idx_job_match_overrides_job_id ON job_match_overrides(job_id);
CREATE INDEX idx_job_match_overrides_source ON job_match_overrides(source_system, source_key);
CREATE INDEX idx_job_assets_job_id ON job_assets(job_id);
CREATE INDEX idx_job_assets_type ON job_assets(type);
CREATE INDEX idx_job_assets_is_active ON job_assets(is_active);
CREATE INDEX idx_major_assets_major_id ON major_assets(major_id);
CREATE INDEX idx_major_assets_type ON major_assets(type);
CREATE INDEX idx_major_assets_is_active ON major_assets(is_active);
CREATE INDEX idx_job_dict_codes ON job_dictionary_index(d_job_cd, d_job_cd_seq);
CREATE INDEX idx_job_dict_seeded ON job_dictionary_index(is_seeded);
CREATE INDEX idx_job_dict_job_nm ON job_dictionary_index(job_nm);
CREATE INDEX idx_job_dict_class ON job_dictionary_index(large_class_cd, medium_class_cd, small_class_cd);
CREATE INDEX idx_seed_logs_type ON seed_logs(seed_type);
CREATE INDEX idx_seed_logs_status ON seed_logs(status);
CREATE INDEX idx_seed_logs_started ON seed_logs(started_at DESC);
CREATE INDEX idx_seed_errors_type ON seed_errors(seed_type, entity_type);
CREATE INDEX idx_seed_errors_retry ON seed_errors(retry_count, resolved_at);
CREATE INDEX idx_seed_errors_entity ON seed_errors(entity_type, entity_id);
...
CREATE INDEX idx_feedback_posts_is_private ON feedback_posts (is_private);
CREATE INDEX idx_feedback_posts_type ON feedback_posts (type);
CREATE UNIQUE INDEX idx_feedback_replies_post ON feedback_replies (post_id);
CREATE INDEX idx_name_mappings_type ON name_mappings(type);
CREATE INDEX idx_name_mappings_source ON name_mappings(source_name);
CREATE INDEX idx_name_mappings_target ON name_mappings(target_name);
CREATE INDEX idx_name_mappings_active ON name_mappings(is_active);
CREATE INDEX idx_name_mappings_type_active ON name_mappings(type, is_active);
CREATE INDEX idx_howto_reports_page ON howto_reports(page_id);
CREATE INDEX idx_howto_reports_status ON howto_reports(status);
CREATE INDEX idx_howto_reports_reporter ON howto_reports(reporter_id);
CREATE INDEX idx_howtos_slug ON howtos(slug);
CREATE INDEX idx_howtos_author ON howtos(author_id);
CREATE INDEX idx_howtos_status ON howtos(status);
CREATE INDEX idx_howtos_updated_at ON howtos(updated_at DESC);
CREATE INDEX idx_howtos_created_at ON howtos(created_at DESC);
CREATE INDEX idx_drafts_user ON howto_drafts(user_id);
CREATE INDEX idx_drafts_stage ON howto_drafts(stage);
CREATE INDEX idx_drafts_base ON howto_drafts(base_howto_id);
CREATE INDEX idx_drafts_updated_at ON howto_drafts(updated_at DESC);
CREATE INDEX idx_tags_slug ON tags(slug);
CREATE INDEX idx_tags_usage ON tags(usage_count DESC);
CREATE INDEX idx_slug_history_old ON slug_history(old_slug);
CREATE INDEX idx_slug_history_howto ON slug_history(howto_id);
CREATE INDEX idx_reviews_draft ON howto_reviews(draft_id);
CREATE INDEX idx_reviews_reviewer ON howto_reviews(reviewer_id);
CREATE INDEX idx_uploads_user ON uploaded_files(user_id);
CREATE INDEX idx_uploads_key ON uploaded_files(file_key);
CREATE INDEX idx_uploads_howto ON uploaded_files(used_in_howto_id);
CREATE INDEX idx_uploads_draft ON uploaded_files(used_in_draft_id);
...
CREATE INDEX idx_user_bookmarks_slug ON user_bookmarks(item_slug);
CREATE INDEX idx_drafts_published_page ON howto_drafts(published_page_id);
CREATE INDEX idx_pages_slug ON pages(slug);
CREATE INDEX idx_pages_type ON pages(page_type);
CREATE INDEX idx_pages_status ON pages(status);
CREATE INDEX idx_pages_author ON pages(author_id);
CREATE INDEX idx_page_relations_page_id ON page_relations(page_id);
CREATE INDEX idx_page_relations_related ON page_relations(related_page_id);
...
CREATE INDEX idx_jobs_primary_source ON jobs(primary_source);
CREATE INDEX idx_jobs_slug ON jobs(slug);
CREATE INDEX idx_majors_name ON majors(name);
CREATE INDEX idx_majors_is_active ON majors(is_active);
CREATE INDEX idx_majors_primary_source ON majors(primary_source);
...
CREATE INDEX idx_jobs_name_en ON jobs(name_en);
CREATE INDEX idx_majors_name_en ON majors(name_en);
CREATE INDEX idx_jobs_image_prompt ON jobs(image_prompt);
CREATE INDEX idx_majors_image_prompt ON majors(image_prompt);
CREATE INDEX idx_requests_session ON ai_analysis_requests(session_id);
CREATE INDEX idx_requests_status ON ai_analysis_requests(status);
CREATE INDEX idx_results_request ON ai_analysis_results(request_id);
CREATE INDEX idx_followup_request ON followup_responses(request_id);
CREATE INDEX idx_followup_constraint ON followup_responses(constraint_type);
CREATE INDEX idx_confirmed_session ON confirmed_constraints(session_id);
CREATE INDEX idx_confirmed_user ON confirmed_constraints(user_id);
CREATE INDEX idx_qh_session ON question_history(session_id);
CREATE INDEX idx_qh_request ON question_history(request_id);
CREATE INDEX idx_job_attr_job_id ON job_attributes(job_id);
CREATE INDEX idx_job_attr_tagger ON job_attributes(tagger_version);
CREATE INDEX idx_job_attr_source ON job_attributes(source_system);
CREATE INDEX idx_job_attr_confidence ON job_attributes(_confidence);
CREATE INDEX idx_job_attr_wlb ON job_attributes(wlb);
CREATE INDEX idx_job_attr_remote ON job_attributes(remote_possible);
CREATE INDEX idx_job_attr_degree ON job_attributes(degree_required);
CREATE INDEX idx_job_attr_license ON job_attributes(license_required);
CREATE INDEX idx_tagger_runs_status ON tagger_runs(status);
CREATE INDEX idx_tagger_runs_version ON tagger_runs(tagger_version);
CREATE INDEX idx_tagger_errors_run ON tagger_errors(run_id);
CREATE INDEX idx_tagger_errors_job ON tagger_errors(job_id);
CREATE INDEX idx_job_attributes_status ON job_attributes(status);
...
CREATE TRIGGER trg_howtos_updated_at
...
END;
CREATE TRIGGER trg_howto_tags_insert
AFTER INSERT ON howto_tags FOR EACH ROW
BEGIN
  UPDATE tags SET usage_count = usage_count + 1 WHERE id = NEW.tag_id;
END;
CREATE TRIGGER trg_howto_tags_delete
