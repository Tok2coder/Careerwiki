...
- 마지막 완료: **R67** (발포고무경화원~방사반장 25직업, 발효/발포/밧줄/방사선/방사성폐기물 계열 — 대부분 niche + 방사선연구원/방사선투과검사기술자 major, 2026-07-01). KPI **2082** (admin job-equalize 단일진실, 2057→2082 +25 정확 일치, admin.ts 쿼리 직접 실측 + verify-cycle 25/25 PASS 이중 확인). max master rev **20110**(opus WARN 수정 5직업 20106~20110 포함, enhance 마지막=방사반장 20105). 25/25 처리. verify-cycle 전수 PASS 25/25(WARN 0/FAIL 0), 마커 25/25, distinct 최소10(niche 10~11/major 18~21), totalE 전건≥19, sal 미접촉. **🔴 사고 1건: B3 세션 API Overloaded 사망** — 4/5 POST 체크포인트 생존, 5번째 방사반장만 미처리(DB master_revs=0 확정). DB로 미생성 확인 후 방사반장 1건만 재spawn(rev 20105, dup 가드) → 25/25 복구, 중복 POST 0. **opus 검증서 urlUnverified 5직업 정책위반 URL 교체**: 발효유균질액혼합원 indeed.com/jobs?q= 검색URL 3건→aTFIS 유제품 세분시장 deep, 방사선연구원 nsis _csrf 세션URL→KOFONS 안전관리, 방사선투과검사기술자·방사성폐기물인수검사원·방사성폐기물자체관리자 nsis /tt/eduG 로그인게이트 stub→KOFONS 안전교육(rev 20106~20110, 마커유지·KPI무영향, false-positive 0). 교훈: indeed/잡포털 검색URL(`/jobs?q=`) + `_csrf` 세션URL은 verify-cycle urlUnverified로 잡히므로 opus 검증서 판별·교체 필수. **(R66 사고 이력: background 전멸→run_in_background:FALSE / abilityList=''→500 hotfix+게이트, abilityList="" 21건 정합성 후속 여전히 미착수.)**
- **🔴🔴 실행모델 (R66 실측 갱신 2026-07-01, 2.1.195): 데몬은 `1 task=1 turn=1 child 수명` — background(`run_in_background` 미지정 또는 `true`)로 띄운 Agent는 메인 turn 종료 시 프로세스와 함께 사망(R66 5배치 2회 전멸). turn 유지(Bash 폴링)해도 이벤트루프 블로킹으로 background 무진행(size 0B). ⚠️ **2.1.195(6/28~)부터 Agent 도구 기본이 async가 됨** — R65(2.1.185)까지 되던 "foreground Agent 자동" 전제가 깨졌다. → **Agent 호출에 `run_in_background:FALSE`를 명시**해야 동기 블로킹으로 subagent 완주 후 결과를 그 turn 안에서 수령(FOREGROUND_SYNC_OK 실증). 한 메시지에 여러 개 `false`로 보내면 병렬 완주. background 배치 절대 금지. ⚠️ foreground 5배치는 그 turn이 길어짐 → compaction 위험, 필요시 cycle 청크.**
- **다음: R68** (R67 다음 가나다 순, 방사반장 이후). `--cycle=68`로 5 배치 prompt 생성. **배치 Agent는 `run_in_background:false` 명시(위 실행모델).** preflight: KPI 2082·max rev 20110 대조. **🔴 go-gate: 매 cycle Jason go 대기, 자동 연속 금지.** 품질 게이트 불변(직업당 validate/audit + opus 검증세션 전수 + **abilityList 타입게이트(신설)** + dispatcher node fetch 거짓양성/진짜dead 판별 + 명단 entity_id 집합검증 master=25 + **prod 페이지 25/25 HTTP 200 fetch 확인**). ⚠️ preflight서 KPI가 2057과 다르면 DB 기준 재구성 후 진행. **⚠️ 중단 시 `--resume=N`으로 미완분만 재배치(완료분 dup 가드 필수).**
