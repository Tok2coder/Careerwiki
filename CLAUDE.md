...
- 마지막 완료: **R61** (반도체광학재료공정연구원~반도체소자웨이퍼검사장비부분품연구원 25직업, 전건 반도체 세부공정직 major, 2026-06-21). KPI **1932** (admin job-equalize 단일진실 기준, 1907→1932 +25 정확 일치, DB 독립 재실측 + 25직업 전수 latest=MASTER 집합검증 master25/25). max master rev **19777**. 25/25 처리. 검증 전수 PASS(opus). **리밋 없이 단판 완주.** Workflow(parallel 5 batch sonnet + verify opus). 반도체 금속막증착·금속재료·사진현상·세정·웨이퍼검사 유사직군 25 — 직업 간 본문/출처 복붙 금지·공정 특화 차별화(기술자 vs 연구원 vs 장비공정 등 역할 분리, Jaccard 점검 통과). distinct 전건 major≥18(최대25), totalE 전건≥19(최소20), 마커 전건 OK, sal 미접촉. dead URL 다수 cleanup(kitech/ETRI/KIMM/samsung deep page 404 → scienceon/ntis/kriss/kaeri 등 한국 1차 대체, B2 12건·B5 3건 등). 반도체세정 cluster urlUnverified WARN(ksia.or.kr·semes.com)은 Chrome-UA 200 거짓양성. prod 3표본 HTTP 200+title 매칭. **verify가 class를 major(반도체)로 정확 표기(prompt 보강 효과).**
- **다음: R62** (R61 다음 가나다 순, 반도체소자웨이퍼검사장비부분품연구원 이후). `--cycle=62`로 5 배치 prompt 생성 필요. preflight: KPI 1932·max rev 19777 대조. **🔴 go-gate 복원 완료(R59~R61 한시 자동연속 종료) — R62부터 매 cycle Jason go 대기, 자동 연속 금지.** 품질 게이트 불변(직업당 validate/audit + 검증세션 전수 + dispatcher node fetch 거짓양성/진짜dead 판별 + 명단 entity_id 집합검증 master=25). ⚠️ preflight서 KPI가 1932와 다르면 DB 기준 재구성 후 진행. **⚠️ 리밋 중단 시 `--resume=N`으로 미완분만 재배치(완료분 dup 가드 필수) — 정착된 수습 패턴.**
