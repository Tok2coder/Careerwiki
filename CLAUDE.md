...
- 마지막 완료: **R58** (바렐기조작원~바이오의약품완제포장원 25직업, 바·바이오 계열, 2026-06-21). KPI **1857** (admin job-equalize 단일진실 기준, 1832→1857 +25 정확 일치, DB 독립 재실측 + 25직업 전수 latest=MASTER 집합검증 master25/25). max master rev **19612**. 25/25 처리. 검증 전수 PASS(opus). **리밋 없이 단판 완주.** Workflow(parallel 5 batch sonnet + verify opus) 오케스트레이션. **검증세션이 바이오농약제조원 dead namu.wiki 404 3건 직접 교체 cleanup(trivia Bt→ko.wiki·농약관리법→psis.rda.go.kr·way 유기농업기사→gnu.ac.kr 전부 200, +wikiQuota 30% 준수 1건 KREI PDF, rev 19612, urlDead 0). KPI cleanup이라 1857 불변(이미 master 마커·latest 여전히 job-data-master).** distinct 전건(niche≥10 최소13 바이오농약제조원/나머지24 ≥18, 세라믹 최대23), totalE 전건≥19(최소22), 마커 전건 OK, sal 미접촉. prod 3표본 HTTP 200+title 매칭. **⚠️ B3 워커 자가보고 슬러그 오기재(소결원↔포장원) — DB 집합검증으로 25 distinct entity_id 전건 무결 확인(자가보고 불신·DB 진리 원칙).**
- **다음: R59** (R58 다음 가나다 순, 바이오의약품완제포장원 이후). `--cycle=59`로 5 배치 prompt 생성 필요. preflight: KPI 1857·max rev 19612 대조. **go-gate — 매 cycle go 대기(자동 연속 안 함).** 품질 게이트 불변(직업당 validate/audit + 검증세션 전수 + dispatcher node fetch 거짓양성/진짜dead 판별 + 활동행 6유닛 done 점검 + 명단 entity_id 집합검증 master=25). ⚠️ preflight서 KPI가 1857와 다르면 DB 기준 재구성 후 진행. **⚠️ 리밋 중단 시 `--resume=N`으로 미완분만 재배치(완료분 dup 가드 필수) — 정착된 수습 패턴.**
