...
- 마지막 완료: **R56** (문지제작반장~미생물발효연구원 25직업, 2026-06-20). KPI **1807** (admin job-equalize 단일진실 기준, 1782→1807 +25 정확 일치, DB 독립 재실측 + 25직업 전수 latest=MASTER 집합검증 total25/master25). max master rev **19508**. 25/25 처리. 검증 전수 PASS(opus). **⚠️ 1차 5배치 session limit 동시 중단(4:20pm 리셋, 8/25만 landing) → DB 실측으로 미완 17직업 식별, `--resume=56`로 미완분만 재배치 완주(완료분 8직업 dup 가드+직업당 POST 전 latest 마커 재확인 → 중복 POST 0 확인).** 검증서 koti.re.kr deep path 소멸 dead URL 교체(물류기술연구원 rev19507·물류전산조작원 rev19508, `.jsp`→`.do` 생존 deep, KPI 무영향). 문학연구원 dup rev 1건(19485 enhance+19486 nrf.re.kr dead 수습 cleanup, 정당·룰C 잉여 아님, latest 마커 OK). 잔존 WARN(artinsight·kospo·koenergy·dongasocio TLS leaf·law.go.kr·koti anti-bot) 거짓양성. distinct 전건(niche≥10 최소10/major≥18), totalE 전건≥19, sal 미접촉. sonnet ×5 bg Agent(+재개 5) + opus 검증. R55 URL 생존 룰(룰17 강조) prompt 주입 — B1 환각 재발 0.
- **다음: R57** (R56 다음 가나다 순, 미생물발효연구원 이후). `--cycle=57`로 5 배치 prompt 생성 필요. preflight: KPI 1807·max rev 19508 대조. **go-gate — 매 cycle go 대기(자동 연속 안 함).** 품질 게이트 불변(직업당 validate/audit + 검증세션 전수 + dispatcher node fetch 거짓양성/진짜dead 판별 + 활동행 6유닛 done 점검). ⚠️ preflight서 KPI가 1807과 다르면 DB 기준 재구성 후 진행. **⚠️ 리밋 중단 시 `--resume=N`으로 미완분만 재배치(완료분 dup 가드 필수).**
