...
6. 매뉴얼 절차 따라: `--next-cycle`(또는 `--cycle=N`) 생성 → **5건-1세션 ×5 배치 (sonnet, `run_in_background:false` 한 메시지 동시 spawn). 건당 순차 POST 체크포인트(idempotent 경계)** → 결과 수집 → **검증 세션 1 (opus — 결정적 게이트 + {master|major}-verify-cycle 전수 실측)** → 보고 → 종료 갱신 2곳 + 사용자 ping (자동 다음 cycle X)

진행 상태 요약 — **스냅샷 금지, 포인터만 (SSOT 정리 2026-07-02)**:
- 마지막 완료 cycle·KPI·max rev·다음 cycle 번호는 **여기 적지 않는다** — `--status`(DB)와 dispatcher 메모리 복제본(`~/Downloads/memory_replica_careerwiki_*_progress.md`)이 진리. (이 자리 스냅샷 stale이 R41 중복 재시작 직전·R45 drift 사고 유발)
- **절차 SSOT = `data/cycle/_dispatcher_manual.md` 1곳** (도구매핑·실행모델·수습 패턴·함정 아카이브·전공 M-cycle 치환표). 핵심 안전룰만 상주: 배치/검증 Agent **`run_in_background:false` 명시**(2.1.195+ async 기본 — background 전멸 사고) / **go-gate 매 cycle, 자동 연속 금지** / KPI는 admin 단일진실 쿼리만(helper A/B 보고 금지) / 중단 시 `--resume=N`(완료분 dup 가드) / 명단 entity_id 집합검증이 완료 게이트.
- **전공 M-cycle (2026-07-02 하네스 빌드 완료)**: 설계서 `data/cycle/major_cycle_design_v1.md` + 매뉴얼 §M 치환표. helper `scripts/major-cycle-helper.cjs` / 명단 `data/cycle/major_list_M0_M25.jsonl`(607전공, M0 파일럿5+M1~M25) / 스킬 major-data-master / 마커 `[major-data-master]`. 다음: M0 파일럿(go 대기).
- master_list(직업): `data/cycle/master_list_R7_R229.jsonl` (223 cycle / 5575 직업).
