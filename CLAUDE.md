...
- 마지막 완료: **R63** (반도체이온주입장비부분품연구원~칩본딩공정연구원 25직업, 반도체 장비·재료·칩몰딩·칩본딩 계열 전건 major, 2026-06-23). KPI **1982** (admin job-equalize 단일진실 기준, 1957→1982 +25 정확 일치, DB 독립 재실측 + 25직업 전수 latest=MASTER 집합검증 master25/25). max master rev **19888**. 25/25 처리. 검증 전수 PASS(opus). distinct 전건≥18(최소18), totalE 전건≥19(최소20), 마커 전건 OK, sal 미접촉. 복붙 Jaccard 6쌍 ≤15.5%(역할분리). opus 검증 중 dead URL 5직업 수정(etnews 404·KTL 404·Amkor 사이트개편 404·ksms.or.kr=버섯학회 오귀속→kims.re.kr·중복 samsung URL→amkor) URL only 교체, KPI 유지. **⚠️ 사고1: 제품기획관리자 abilityList:"" 빈문자열→Workers `"".map` 500→hotfix(abilityList:null) 200 복구. ⚠️ 사고2(중대): 데몬 프로세스가 ~15~25분마다 재시작돼 background 배치가 4~5회 반복 사망("previous Claude Code process exited"). --resume 반복으로 4→9→15→22→25 누적 완주, 마지막 3직업은 foreground Agent로 무사 완주. ⚠️ 잠재이슈: 칩몰딩 공정기술자/연구원 출처 hrd.go.kr→work24.go.kr(origin 금지) 301 리다이렉트, 차기 cleanup 권장.**
- **🔴🔴 R64+ 실행모델 전환 (서연이 데몬코드서 원인확정, 2026-06-23): 데몬은 `1 task=1 turn=1 child 수명` — dispatcher가 `run_in_background:true`로 띄운 Workflow/Agent는 메인 turn 종료 시 프로세스와 함께 죽는다(R63서 4~5회 사망 원인). 반면 heartbeat가 active turn lease를 30초마다 갱신 → `run_in_background` 없는(foreground) Agent는 1시간 long turn도 데몬이 안 죽인다(R63 마지막 3직업 foreground로 무사 완주 검증). → 배치는 foreground Agent로 띄워 결과를 그 turn 안에서 수령(spawn 원칙 유지=직접 작업 아님). cycle을 청크로 끊어 turn 과도히 길지 않게(compaction 회피). background 배치 금지.**
- **다음: R64** (R63 다음 가나다 순, 반도체칩본딩공정연구원 이후 = 반도체칩본딩장비공정연구원부터). `--cycle=64`로 5 배치 prompt 생성. preflight: KPI 1982·max rev 19888 대조. **🔴 go-gate: 매 cycle Jason go 대기, 자동 연속 금지.** 품질 게이트 불변(직업당 validate/audit + opus 검증세션 전수 + dispatcher node fetch 거짓양성/진짜dead 판별 + 명단 entity_id 집합검증 master=25). ⚠️ preflight서 KPI가 1982와 다르면 DB 기준 재구성 후 진행. **⚠️ 중단 시 `--resume=N`으로 미완분만 재배치(완료분 dup 가드 필수).**
