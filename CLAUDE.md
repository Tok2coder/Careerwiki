...
- 마지막 완료: **R62** (반도체소자패키지검사공정기술자~이온주입장비기술자 25직업, 전건 반도체 세부공정직 major, 2026-06-22). KPI **1957** (admin job-equalize 단일진실 기준, 1932→1957 +25 정확 일치, DB 독립 재실측 + 25직업 전수 latest=MASTER 집합검증 master25/25). max master rev **19830**. 25/25 처리. 검증 전수 PASS(opus). distinct 전건≥18(최소18), totalE 전건≥24(최소24, 임계19 여유), 마커 전건 OK, sal 미접촉(UCJ 25건 전부 overviewSalary 키 부재). 본문 shingle Jaccard 최대 0.146(복붙 없음, 연구원=R&D/석사/장비공정 vs 기술자=유지보수/Field Engineer 역할분리). 이온주입장비 공정연구원↔기술자 source-URL Jaccard 0.85는 동일 niche(Axcelis·삼성DS·SETEC·K-CHIPS) 공유 토픽 자연스러움(본문은 차별화). URL liveness 표본 19건 200, NXDOMAIN/dead 0. jobfair-gri.kr 인증서만료·semi.org 403 anti-bot 거짓양성 처리. prod 3표본 200+키워드. **⚠️ 사고: Workflow 백그라운드가 2회 죽음(1차 launch 0건, 2차 14직업서 31분 후 무응답 중단) → `--resume=62`로 미완 11직업만 재배치, dup가드(POST 전 latest 마커 재확인)로 중복 0, resume Workflow ~56분 완주.**
- **다음: R63** (R62 다음 가나다 순, 반도체이온주입장비기술자 이후). `--cycle=63`으로 5 배치 prompt 생성 필요. preflight: KPI 1957·max rev 19830 대조. **🔴 go-gate: 매 cycle Jason go 대기, 자동 연속 금지.** 품질 게이트 불변(직업당 validate/audit + 검증세션 전수 + dispatcher node fetch 거짓양성/진짜dead 판별 + 명단 entity_id 집합검증 master=25). ⚠️ preflight서 KPI가 1957과 다르면 DB 기준 재구성 후 진행. **⚠️ 리밋/Workflow중단 시 `--resume=N`으로 미완분만 재배치(완료분 dup 가드 필수) — 정착된 수습 패턴. Workflow 백그라운드는 장시간 작업 중 죽을 수 있으니 launched 응답 신뢰 말고 생존 모니터(agent jsonl mtime + journal completed + DB POST 진척) 필수.**
