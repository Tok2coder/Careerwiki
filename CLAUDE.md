...
- 마지막 완료: **R64** (반도체칩본딩장비공정연구원~화학적기계적연마장비기술자 25직업, 반도체 칩본딩·칩절단·칩접합·CVD·CMP 계열 전건 major, 2026-06-23). KPI **2007** (admin job-equalize 단일진실 기준, 1982→2007 +25 정확 일치, DB 독립 재실측 2회 + 25직업 전수 latest=MASTER 집합검증 master25/25). max master rev **19942**. 25/25 처리. 검증 전수 PASS(opus). distinct 전건≥18(최소18, B4 4직업 12~13→18~20 보강), totalE 전건≥19(최소29), 마커 전건 OK, sal 미접촉, abilityList 빈문자열 0(R63 사고 미재발). 복붙 Jaccard 6쌍 9~29%(역할분리). opus 검증 중 dead URL 4직업 수정(화학약품재료개발연구원 krict soft-404·kims stub 200 → 라이브 동등페이지). **★ R64는 foreground 5배치 병렬 + foreground opus 검증으로 죽음 0회 무중단 완주 — R64+ 실행모델 실증 완료.** **⚠️ repo `url-liveness.cjs`(HEAD 검사)가 soft-404/stub 200을 OK로 오판하는 맹점 발견 → node GET+본문검사로 backstop 필요. ⚠️ 잠재(차기 cleanup): fliphtml5.com(KSIA flipbook)·careers-page.com 준-decorative host 반복.**
