...
- 마지막 완료: **R66** (발전기계공무기술원~발전효율관리원 25직업, 발전 계열 major 중심 + niche 소수, 2026-07-01). KPI **2057** (admin job-equalize 단일진실, 2032→2057 +25 정확 일치, DB 재실측 + 25직업 전수 마커 집합검증 25/25). max master rev **20050**. 25/25 처리. opus 검증 전수 PASS(audit 9패턴 FAIL 0, distinct major≥18·niche 발전업무보조원=10, totalE 24건≥21·발전업무보조원=19 하한통과, 마커 25/25, sal 미접촉, 복붙 Jaccard 전건<16%). **🔴 사고 2건: (1) background 5배치 2회 전멸 — 2.1.195(6/28~)부터 Agent 도구 기본이 async라 `run_in_background` 미지정/true는 데몬 turn 종료 시 사망. `run_in_background:FALSE` 명시로 foreground 동기완주 해결(FOREGROUND_SYNC_OK 실증). (2) 배치가 `overviewAbilities.abilityList`를 `""`(빈문자열)로 저장 → createJobJsonLd(src/templates/unifiedJobDetail.ts:5484) `?.map` 옵셔널체이닝 통과 → `"".map is not a function` TypeError → JSON-LD 조립부(try-catch 없음) → 20직업 HTTP 500. 렌더 Array.isArray 가드 hotfix(main bd4625f 배포)로 25/25 200 복구 + validate-job-edit.cjs abilityList 타입게이트 신설(재발 원천차단). ⚠️ abilityList="" 데이터 21건 잔존(렌더 방어됨, 500 없음 — 정합성 후속: editor ""→null 정규화 + 재정리 미착수).**
